...
class Operationalreport extends Base {
	...
	function index()
	{
		...
		if ($privilegecode == 1) {
			...
			$this->params["content"]        = $this->load->view('newdashboard/report/vopr_report', $this->params, true);
...
//		ini_set('memory_limit', '2G');
		...
		//$report = "operasional_";
		$report = "operasional_location_"; // new report
		$report_sum = "summary_";
		...
		$rom_list = array("ROM 01","ROM 02","ROM 03","ROM 04","ROM 05","ROM 06","ROM 07","ROM 08");
		...
		switch ($m2)
		{
			...
			if($vehicle != "all"){
				$this->dbtrip->where("trip_mileage_vehicle_id", $vehicle);
			}
			...
			$this->dbtrip->where("trip_mileage_duration_sec >=", 60);
			if($type_km == "1"){
				if($km_start != "" && $km_end != ""){
			...
			if($geotype != "all"){
				if($geotype == "road"){
					$this->dbtrip->where("trip_mileage_location_start <>", "POOL KMB 2");
					$this->dbtrip->where("trip_mileage_location_start <>", "WS KMB");
					$this->dbtrip->like("trip_mileage_location_start", "KM");
					$this->dbtrip->where("trip_mileage_km_status",1);
					//$this->dbtrip->where("trip_mileage_geofence_start_type", $geotype);
				}else if($geotype == "out"){
					$this->dbtrip->where("trip_mileage_geofence_start_type", $geotype);
				}else if($geotype == "site"){
					$this->dbtrip->where_in("trip_mileage_location_start", $rom_list);
					$this->dbtrip->where_in("trip_mileage_location_end", $rom_list);
					$this->dbtrip->where("trip_mileage_speed_status_rev", "low");
					$this->dbtrip->where("trip_mileage_km >= ", 0);
				}else if($geotype == "port"){
					$this->dbtrip->where_in("trip_mileage_location_start", $port_list);
					$this->dbtrip->where_in("trip_mileage_location_end", $port_list);
					$this->dbtrip->where("trip_mileage_speed_status_rev", "low");
					$this->dbtrip->where("trip_mileage_km >= ", 0);
				}
				else{
			...
			if($statusname != "all"){
				/*if($statusname == "STOP"){
					...
					$this->dbtrip->where("trip_mileage_trip_mileage", 0);
				}else{
					$this->dbtrip->where("trip_mileage_status", $statusname);
				}*/
				if($statusname == "BREAKDOWN"){
					$breakdown_list = array("MOVE2","IDLE2");
					$this->dbtrip->where_in("trip_mileage_status", $breakdown_list);
					$this->dbtrip->where("trip_mileage_km >= ", 0);

				}else if($statusname == "MOVE"){
					$this->dbtrip->where("trip_mileage_speed_avg_rev <= ", 80);
					$this->dbtrip->where("trip_mileage_status", $statusname);
				}
