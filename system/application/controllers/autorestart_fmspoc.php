...
class Autorestart_fmspoc extends Base {
	var $otherdb;
	function Autorestart_fmspoc()
	{
		parent::Base();
	...
	// alert Port to Port with WA & extend ritase fraud + 10 menit + REDZONE SAWITAN + COMPARE LAST ROM VS KM ATAS + DARI ROM KELUAR GEOFENCE + KM BAWAH + ODOMETER AUTOCHECK
	// redzone , warning zone , fraud (share ke Mitra)
	function autocheck_new($groupname="", $userid="4408", $order="asc")
	{
		...
		$this->db = $this->load->database("fmspoc_default", TRUE);
		...
		$NON_DT_list_company = array("1962","1961");
		...
		for($x=0;$x<$totalmodem;$x++)
		{
			...
			//$hcode_list = array("AEK 0031","AEK 0129");
			...
			//$this->db->where_in("vehicle_no", $hcode_list); //tes only
			...
			if($total_rows>0){
				...
				for($i=0;$i<$total_rows;$i++)
				{
					...
					$vehicle_imeicam = $rowvehicle[$i]->vehicle_mv03;
					...
					$lastkmatasname = $rowvehicle[$i]->vehicle_km_atas_name;
					$lastkmatastime = $rowvehicle[$i]->vehicle_km_atas_time;
					$lastkmbawahname = $rowvehicle[$i]->vehicle_km_bawah_name;
					$lastkmbawahtime = $rowvehicle[$i]->vehicle_km_bawah_time;
					...
					$gps = $this->getlastposition_fromDBLive($vehicle_imei,$vehicle_dblive);
						...
						if(count($gps)>0){
							...
							$driver_detected_data = $this->get_driver_detected($vehicle_imeicam);
							if(count($driver_detected_data)>0){
								
								$driver_name = $driver_detected_data->change_driver_name;
								$driver_id = strtoupper($driver_detected_data->change_driver_id);
								$driver_time = $driver_detected_data->change_driver_time;
								printf("===Driver Detected %s %s %s \r\n", $driver_id, $driver_name, $driver_time);
							}
							else{
								printf("===No Data Driver %s \r\n", $vehicle_no);
							}
										...
										$km_atas_register = $this->config->item('km_atas_register_autocheck');
										$km_bawah_register = $this->config->item('km_bawah_register_autocheck');
							...
							if(isset($lastposition)){
								...
								if (in_array($street_name, $street_register)){
									...
									// NOT in STREET ON DUTY (HAULING, ROM, PORT, WS)
									$hauling = "out";
									if($speed == 0){
										$outHAULING_status = 1;
									}
								...
								printf("===Location %s, Jalur %s , Hauling %s \r\n", $street_name, $jalur, $hauling); //print_r("DISINI");exit();
										...
										$redzone_sawitan = $this->config->item('redzone_sawitan_autocheck');
										...
										$auto_km_atas_name = $lastkmatasname;
										$auto_km_atas_time = $lastkmatastime;

										$auto_km_bawah_name = $lastkmbawahname;
										$auto_km_bawah_time = $lastkmbawahtime;

										printf("===LAST ROM %s %s \r\n", $auto_last_rom_name, $auto_last_rom_time);
										printf("===LAST PORT %s %s \r\n", $auto_last_port_name, $auto_last_port_time);
										printf("!==Unit terdetek ke ROM: %s %s KM ATAS: %s %s \r\n",$auto_last_rom_name, $auto_last_rom_time, $auto_km_atas_name, $auto_km_atas_time);
											...
											else
											{
												...
												else
												{
													...
													if (in_array($auto_last_rom_name, $rombib_register))
													{
														...
														$attachmentlink = "https://fmspoc.abditrack.com/attachment/playbackhistory/".$vehicleidfix.'/'.$date_x.'/'.$date_y;
														$redzone_status = 1;
														...
														else
														{
															...
															printf("===SENT REDZONE BIB OK\r\n");

															// $company_telegram_red = $this->getTelegramID_nonbib($vehiclecompany);
															if(count($company_telegram_red)>0){
																$telegram_redzone = $company_telegram_red->company_telegram_redzone;
																//$sendtelegram_red = $this->telegram_direct($telegram_redzone,$message); //telegram MITRA REDZONE
																printf("===SENT REDZONE MITRA OK\r\n");
															}else{
																printf("X==NOT SENT REDZONE MITRA, TELE ID NULL OK\r\n");
															}
										...
										//red zone SAWITAN
										if (in_array($street_name, $redzone_sawitan))
										{
											...
											else
											{
												...
												else
												{
													...
													if (in_array($auto_last_rom_name, $rombib_register))
													{
														...
														$attachmentlink = "https://fmspoc.abditrack.com/attachment/playbackhistory/".$vehicleidfix.'/'.$date_x.'/'.$date_y;
														$redzone_status = 1;
														$title_name = "REDZONE SAWITAN DETECTED!!";
														$message = urlencode(
																	"".$title_name." \n".
																	"Time: ".$gps_realtime." \n".
																	"Vehicle No: ".$rowvehicle[$i]->vehicle_no." \n".
														...
														if($lastrom_text == "No Data ROM" || $lastport_text == "No Data PORT"){
															...
															//$notif_wa_redzone = $this->sendnotif_wa_redzone($wa_token,$gps_realtime,$rowvehicle[$i]->vehicle_no,$street_name,$url,$speed,$lastrom_text,$lastport_text,$attachmentlink);
															//$sendtelegram = $this->telegram_direct("-657527213",$message); //telegram BIB REDZONE
															//$sendtelegram = $this->telegram_direct("-960290788",$message); //telegram BIB REDZONE [TEMP]
															...
															if(count($company_telegram_red)>0){
																$telegram_redzone = $company_telegram_red->company_telegram_redzone;
																//$sendtelegram_red = $this->telegram_direct($telegram_redzone,$message); //telegram MITRA REDZONE (temp matikan sampai spike stabil)
																printf("===SENT REDZONE MITRA OK\r\n");
															}else{
																printf("X==NOT SENT REDZONE MITRA, TELE ID NULL OK\r\n");
															}
								...
								//KM atas > KM 13
								if (in_array($street_name, $km_atas_register))
								{
									//update field master data km atas time
									$wim_time = $this->km_atas_updatetime($rowvehicle[$i],$street_name, $gps_realtime);

								}

								//KM Bawah s/d KM 1
								if (in_array($street_name, $km_bawah_register))
								{
									//update field master data km atas time
									$wim_time = $this->km_bawah_updatetime($rowvehicle[$i],$street_name, $gps_realtime);
								...
								$overspeed_status = 0;
									...
									if($warningzone_status == 1)
									{
										...
										//$notif_wa_warningzone = $this->sendnotif_wa_warningzone($wa_token,$gps_realtime,$rowvehicle[$i]->vehicle_no,$street_name,$url,$speed,$lastrom_text,$lastport_text); (disable sementara)
											...
											// $company_telegram_warning = $this->getTelegramID_nonbib($vehiclecompany);
											if(count($company_telegram_warning)>0){
												$telegram_warning = $company_telegram_warning->company_telegram_warning;
												//$sendtelegram_warning = $this->telegram_direct($telegram_warning,$message); //telegram MITRA WARNING ZONE
													printf("===SENT WARNING ZONE MITRA OK\r\n");
											}else{
												printf("X==NOT SENT WARNING ZONE MITRA, TELE ID NULL OK\r\n");
											}
								...
								if (in_array($vehiclecompany, $DT_list_company))
								{
									if($inPORT_status)
									{
										...
										$limit_periode = 3600 + 3600 + 600; //2.1 jam
										...
										$last_km_atas = strtotime($auto_km_atas_time);
										...
										if($delta_port_new > $limit_port_time)
										{
											...
											else
											{
												...
												//jika Last ROM < Last KM Atas maka potensi spike di last ROM
												if($lastrom_time_new < $last_km_atas)
												{
													$fraud_anomali = 1;
												}
												...
												$date_y_new = $now_time_new+3600+300; //extend 5 menit (300 s)
												//$attachmentlink = "http://attachment.pilartech.co.id/attachment/playbackhistory/".$vehicleidfix.'/'.$date_x_new.'/'.$date_y_new;
												$attachmentlink = "https://fmspoc.abditrack.com/attachment/playbackhistory/".$vehicleidfix.'/'.$date_x_new.'/'.$date_y_new;
												$redzone_status = 1;
												...
												$message = urlencode(
													...
													if($fraud_anomali == 0)
													{
														...
														//$sendtelegram = $this->telegram_direct("-960290788",$message); //telegram BIB POTENTIAL FRAUD [TEMP]

															// $company_telegram_fraud = $this->getTelegramID_nonbib($vehiclecompany);
															if(count($company_telegram_fraud)>0){
																$telegram_fraud = $company_telegram_fraud->company_telegram_fraud;
																//$sendtelegram_fraud = $this->telegram_direct($telegram_fraud,$message); //telegram MITRA POTENTIAL FRAUD (matikan sampai tidak spike)
																printf("===SENT FRAUD MITRA OK\r\n");
															}else{
																printf("X==NOT SENT FRAUD MITRA, TELE ID NULL OK\r\n");
															}

														//disable sampai stabil tidak spike
														//$notif_wa_ritase_fraud = $this->sendnotif_wa_fraud_link($wa_token,$title_name,$gps_realtime,$rowvehicle[$i]->vehicle_no,$street_name,$url,$speed,$lastrom_text_new,$lastport_text_new,$attachmentlink);
													...
													else
													{
														...
														//$notif_wa_ritase_fraud = $this->sendnotif_wa_fraud_link($wa_token,"[TEST] INDIKASI FRAUD RITASE !!",$gps_realtime,$rowvehicle[$i]->vehicle_no,$street_name,$url,$speed,$lastrom_text_new,$lastport_text_new,$attachmentlink);
									...
									//DARI ROM KELUAR GEOFENCE
									if($outHAULING_status == 1)
									{
										$fraud_anomali = 0;
										$limit_rom_time = 30*60; //default 30 menit
										$limit_periode = 3600 + 3600 + 3600; //3 jam
										//$limit_port_time = 3; //test only

										if (in_array($auto_last_rom_name, $rombib_register)) // ROM Legal only
										{
											$lastport_time_new = strtotime($auto_last_port_time);
											$lastrom_time_new = strtotime($auto_last_rom_time);
											$now_time_new = strtotime($gps_realtime);
											$last_km_bawah = strtotime($auto_km_bawah_time);
											$delta_rom_new = $now_time_new - $lastrom_time_new;
											printf("X==Delta Last ROM %s s \r\n", $delta_rom_new);

											$now_loc_date = date("Y-m-d", strtotime($now_time_new));
											$last_rom_date_new = date("Y-m-d", strtotime($lastrom_time_new));

												//DATA PORT > data LAST ROM maka aman, unit sudah dumping
												if($lastport_time_new > $lastrom_time_new)
												{
													printf("X==AMAN Unit terdetek ke PORT: %s %s ASAL ROM: %s %s \r\n",$auto_last_port_name,$auto_last_port_time,$auto_last_rom_name,$auto_last_rom_time);
												}
												...
												else
												{
													...
													//jika ROM to NOW lebih dari limit periode asumsi rom baru)
													if($delta_rom_new > $limit_periode)
													{
														$fraud_anomali = 1;
													}

													//jika durasi last rom dan now time kurang dari limit maka skip notif
													if($delta_rom_new < $limit_rom_time)
													{
														$fraud_anomali = 1;
													}

													//jika beda tanggal NOW & ROM juga asumsi anomali
													if($now_loc_date != $last_rom_date_new)
													{
														$fraud_anomali = 1;
													}

													//jika Last PORT < Last KM Bawah maka potensi spike di last PORT
													if($lastport_time_new < $last_km_bawah)
													{
														$fraud_anomali = 1;
													}
													...
													printf("F==Dari ROM Keluar Geofence %s NOW: %s %s delta: %s \r\n", $lastrom_text_new, $street_name, $gps_realtime, $delta_rom_new/3600);

													//send notif fraud ROM
													$date_x_new = $lastrom_time_new+3600;
													$date_y_new = $now_time_new+3600;
													//$attachmentlink = "http://attachment.pilartech.co.id/attachment/playbackhistory/".$vehicleidfix.'/'.$date_x_new.'/'.$date_y_new;
													$attachmentlink = "https://fmspoc.abditrack.com/attachment/playbackhistory/".$vehicleidfix.'/'.$date_x_new.'/'.$date_y_new;

													$title_name = "DARI ROM KELUAR GEOFENCE !!";
													$message = urlencode(
															"".$title_name." \n".
															"Time: ".$gps_realtime." \n".
															"Vehicle No: ".$rowvehicle[$i]->vehicle_no." \n".
														...
														if($fraud_anomali == 0)
														{
															//$sendtelegram = $this->telegram_direct("-657527213",$message); //telegram BIB GEOFENCE ALERT
															//$notif_wa_ritase_fraud = $this->sendnotif_wa_fraud_link($wa_token,$title_name,$gps_realtime,$rowvehicle[$i]->vehicle_no,$street_name,$url,$speed,$lastrom_text_new,$lastport_text_new,$attachmentlink);
														...
														{
															//$sendtelegram = $this->telegram_direct("-657527213",$message); //telegram FRAUD TEST
														}


														printf("F==SENT TELE FRAUD ROM");
												}
										}
							...
							if ($delta >= 1800 && $delta <= 86400) //default 1jam(3600) -> 24jam(86400)
							{
								...
								array_push($data_k,$info_k);
										...
										if ($qcheck->num_rows() == 0)
										{
											...
											$feature["auto_last_odometer"] = $odometer;
											
											$feature["auto_last_driver_name"] = $driver_name;
											$feature["auto_last_driver_id"] = $driver_id;
											$feature["auto_last_driver_time"] = $driver_time;
							...
							else //gps update condition
							{
								...
								array_push($data_p,$info_p);
										...
										if (in_array($street_name, $nonbib_register))
										{
											...
											//jika bukan DT maka skip notif
											if (in_array($vehiclecompany, $NON_DT_list_company))
											{
												$nonbib_status = 0;
											}
		...
		$sendtelegram = $this->telegram_direct("-875553556",$message); //telegram POC AUTOCHECK
		printf("===SENT TELEGRAM OK\r\n");
	...
	{
		$street_id = 0;
		$this->dbstreet = $this->load->database("fmspoc_default", true);
            ...
            if (isset($rowvehicle[$x]->vehicle_info))
            {
                ...
                if (isset($json->vehicle_ip) && isset($json->vehicle_port))
                {
					...
					else
					{
						...
						$this->dbhist = $this->load->database("fmspoc_default", TRUE);
						$this->dbhist2 = $this->load->database("gpshistory",true);
						//$this->dbhist2 = $this->load->database("gpshistory_rhino",true);
					}
	...
	{
		$georeverse = $this->gpsmodel->GeoReverse_poc($latitude, $longitude);
		return $georeverse;
	}
	...
	function km_atas_updatetime($vehicle,$location,$gpstime){
		$this->db = $this->load->database("fmspoc_default",TRUE);
		$this->db->select("vehicle_id,vehicle_no");
		$this->db->where("vehicle_id",$vehicle->vehicle_id);
		$qvehicle = $this->db->get("vehicle");
		...
		if(count($rvehicle)>0){
			...
			unset($datawim);
			$datawim["vehicle_km_atas_time"] = $gpstime;
			$datawim["vehicle_km_atas_name"] = $location;

			$this->db->where("vehicle_id", $vehicle->vehicle_id);
			$this->db->limit(1);
			$this->db->update("vehicle", $datawim);
			printf("===KM atas EndTime Updated === %s %s %s \r\n", $vehicle->vehicle_no,$location,$gpstime);
			return;
	...
	function km_bawah_updatetime($vehicle,$location,$gpstime){
		$this->db = $this->load->database("fmspoc_default",TRUE);
		$this->db->select("vehicle_id,vehicle_no");
		$this->db->where("vehicle_id",$vehicle->vehicle_id);
		$qvehicle = $this->db->get("vehicle");
		...
		if(count($rvehicle)>0){
			...
			unset($datawim);
			$datawim["vehicle_km_bawah_time"] = $gpstime;
			$datawim["vehicle_km_bawah_name"] = $location;

			$this->db->where("vehicle_id", $vehicle->vehicle_id);
			$this->db->limit(1);
			$this->db->update("vehicle", $datawim);
			printf("===KM bawah EndTime Updated === %s %s %s \r\n", $vehicle->vehicle_no,$location,$gpstime);
			return;
	...
	function outofport_check($userid="",$orderby=""){
		...
		$this->db = $this->load->database("fmspoc_default",true);
		$this->db->order_by("vehicle_id",$orderby);
		$this->db->select("vehicle_id,vehicle_user_id,vehicle_name,vehicle_device,vehicle_no,vehicle_company,vehicle_rom_time,vehicle_rom_name,vehicle_port_time,vehicle_wim_etime,vehicle_dbname_live");
		$this->db->where("vehicle_user_id", $userid);
		...
		if(count($rows)>0){
			...
			$limit_sec = 3600+1300; //1jam -> 1.5 jam //default 3600+1300;
			$limit_sec2 = 3600*3; //1jam 30 menit --> 2 jam */ //defaultnya 2 jam diubah semntara jadi 3 jam setelah dari KM 13
			...
			{	usleep(500);
				...
				printf("===UNIT MASUK KE KM 13 PADA  : %s \r\n",$rows[$i]->vehicle_wim_etime);
				...
				if($totalport > 0){
					...
					if(count($lastposition)>0){
						...
						$lastWIMtime = $rows[$i]->vehicle_wim_etime;
						...
						$lastromtime_sec = strtotime($lastromtime);
						$lastposition_time_sec = strtotime($lastposition_time);
						$lastWIMtime_sec = strtotime($lastWIMtime);

						$delta_WIM_to_port = $lastposition_time_sec - $lastWIMtime_sec;
						//$delta_rom_to_port = $lastposition_time_sec - $lastromtime_sec;

						$duration = get_time_difference($lastWIMtime, $lastposition_time);
						...
						printf("===DELTA ROM TO PORT : %s  %s \r\n", $delta_WIM_to_port, $show);

						//condition status
						if($lastWIMtime < $lastromtime)
						{
							$skip_notif = 1;
						}else{
						...
						if($skip_notif == 0)
						{
							//karena banyak data lompat semntara di range 1 - 1.30 jam --> diubah jadi 1.30 jam - 2 jam
							if($delta_WIM_to_port > $limit_sec && $delta_WIM_to_port < $limit_sec2 )
							{
								...
								if(isset($lastposition_name)){
									...
									if (in_array($street_name, $street_onlocation)){
										...
										//send telegram
										$title_name = "INFORMASI 1.5 JAM BELUM MASUK PORT";
										$message = urlencode(
											"".$title_name." \n".
											...
											printf("===SENT TELEGRAM OK\r\n");
											sleep(2);
											$sendtelegram = $this->telegram_direct("-805319894",$message);
											//$sendtelegram = $this->telegram_direct("-657527213",$message); //FMS TESTING
											printf("===SENT TELEGRAM BIB OK\r\n");

										$lastromname_time = $lastromname." ".$lastromtime;
										//$notif_wa_1hrport_fraud = $this->sendnotif_wa_fraud_nolink($wa_token,$title_name,$lastposition_time,$vehicle_no,$location,$url,$speed,$show,$lastromname_time); //tidak jadi ada notif WA
						...
						{
							printf("X==WIM DATETIME Not Updated !!\r\n");
	...
	function getlastposition_fromDBLive($imei,$dblive){
		...
		$this->dblive = $this->load->database($dblive,true);
		$this->dblive->select("gps_name,gps_time,gps_latitude_real,gps_longitude_real,gps_speed,gps_odometer,
	...
	function get_driver_detected($imei){

		$this->dbts = $this->load->database("webtracking_ts",true);
		$this->dbts->select("change_driver_name,change_driver_id,change_driver_time");
		$this->dbts->order_by("change_driver_time", "desc");
		$this->dbts->where("change_driver_time >", "2023-07-25 00:00:00"); //only setelah akhir POC
		$this->dbts->where("change_driver_name !=", ""); //nama tidak kosong
		$this->dbts->where("change_imei", $imei);
		$this->dbts->limit(1);
		$qpost = $this->dbts->get("ts_driver_change_new");
		$rowpost = $qpost->row();
	...
	function outofhauling_alert($vehicle,$location,$gpstime,$coordinate,$speed){
		...
		$this->dbdefault = $this->load->database("fmspoc_default",true);
	...
	function getTelegramID_nonbib($id)
	{
		...
		$this->db->select("company_id,company_telegram_geofence,company_telegram_speed,company_telegram_redzone,company_telegram_warning,company_telegram_fraud,company_hp");
	...
	function sendnotif_wa_fraud_link($wa_token,$wa_title,$wa_gps_realtime,$wa_vehicle_no,$wa_street_name,$wa_url,$wa_speed,$wa_lastrom_text,$wa_lastport_text,$wa_attachmentlink){
		$mytoken = $wa_token->sess_value;
		...
		$namespace = $this->config->item('WA_NAMESPACE');
		$nametemplate = $this->config->item('WA_TEMPLATE_FRAUD_LINK'); //9 param
		$recipient_dt = $this->config->item('WA_NOMOR_FRAUD');
		...
		for($w=0;$w<$total_recipient;$w++){
			...
			$DataToUpload->template['components'][0]->type = "body";
			$DataToUpload->template['components'][0]->parameters[0]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[0]['text'] = $wa_title;

			$DataToUpload->template['components'][0]->parameters[1]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[1]['text'] = $wa_gps_realtime;
			$DataToUpload->template['components'][0]->parameters[2]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[2]['text'] = $wa_vehicle_no;
			$DataToUpload->template['components'][0]->parameters[3]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[3]['text'] = $wa_street_name;

			$DataToUpload->template['components'][0]->parameters[4]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[4]['text'] = $wa_url;
			$DataToUpload->template['components'][0]->parameters[5]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[5]['text'] = $wa_speed;

			$DataToUpload->template['components'][0]->parameters[6]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[6]['text'] = $wa_lastrom_text;
			$DataToUpload->template['components'][0]->parameters[7]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[7]['text'] = $wa_lastport_text;

			$DataToUpload->template['components'][0]->parameters[8]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[8]['text'] = $wa_attachmentlink;
		...
		printf("!!FRAUD WITH LINK-------- \r\n");
	}

	function sendnotif_wa_fraud_nolink($wa_token,$wa_title,$wa_gps_realtime,$wa_vehicle_no,$wa_street_name,$wa_url,$wa_speed,$wa_duration,$wa_lastrom_text){
		...
		$namespace = $this->config->item('WA_NAMESPACE');
		$nametemplate = $this->config->item('WA_TEMPLATE_FRAUD_NOLINK'); //8 param
		$recipient_dt = $this->config->item('WA_NOMOR_FRAUD');
		...
		for($w=0;$w<$total_recipient;$w++){
			...
			$DataToUpload->template['components'][0]->parameters[1]['text'] = $wa_vehicle_no;
			$DataToUpload->template['components'][0]->parameters[2]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[2]['text'] = $wa_gps_realtime;
			...
			$DataToUpload->template['components'][0]->parameters[6]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[6]['text'] = $wa_duration;
			$DataToUpload->template['components'][0]->parameters[7]['type'] = "text";
			$DataToUpload->template['components'][0]->parameters[7]['text'] = $wa_url;
