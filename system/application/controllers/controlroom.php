...
class controlroom extends Base
{
    ...
    function search()
    {
        ...
        // Konversi tanggal ke format yang sesuai
         ...
         // Tambahkan filter perusahaan jika diberikan
        $where = '';
         if ($company != 0) {
         $where .= " AND vehicle_company = $company";
         }

        // Tambahkan filter pelanggaran jika diberikan
        if (!empty($violation)) {
            $where .= " AND violation_column_name = '$violation'";
        }

          // Query SQL untuk mencari data berdasarkan parameter yang diberikan
         $sql = "SELECT * FROM alarm_report_statusintervention_cr
         WHERE alarm_start_time >= '$startDateTime' AND alarm_end_time <= '$endDateTime'
         AND DATE(alarm_start_time) = '$date' $where";
	...
	function violation2()
	{
		...
		if ($privilegecode == 1) {
			$this->params["sidebar"]        = $this->load->view('newdashboard/partial/sidebar_superuser', $this->params, true);
			$this->params["content"]        = $this->load->view('newdashboard/controlroom/v_daily_violation2', $this->params, true);
			$this->load->view("newdashboard/partial/template_dashboard_superuser", $this->params);
		} elseif ($privilegecode == 2) {
			$this->params["sidebar"]        = $this->load->view('newdashboard/partial/sidebar_managementuser', $this->params, true);
			$this->params["content"]        = $this->load->view('newdashboard/controlroom/v_daily_violation2', $this->params, true);
    ...
    function search_violation2()
	{
		...
		$alarm_evidence_     = "alarm_evidence_";
			...
			if ($numrows > 0) {
				for ($i = 0; $i < $numrows; $i++) {
					if (isset($data_company[$data[$i]['alarm_report_vehicle_company']])) {
                        ...
                        $datetime = $data[$i]['alarm_report_end_time'];
