...
class History_fms_fmspoc extends Base {
 ...
 /*
	...
	*/
	...
	function create_tables($vdevice = "", $type="")
	{
		...
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		$this->db->select("vehicle_device, vehicle_dbhistory_name");
		if ($vdevice != "" && $type != "")
		{
			$vedev = $vdevice."@".$type;
			$this->db->where("vehicle_device", $vedev);
		...
		//$this->db->where("vehicle_device <>", "003100000967@T5");
		$this->db->where("vehicle_user_id", 4408); //khusus indobara
		//$this->db->or_where("vehicle_user_id", 4576); //rmtg
		$q = $this->db->get("vehicle");
		if ($q->num_rows() == 0) return;
		...
		foreach(array("gpshistory", "gpsarchive") as $dbname)
		{
			...
			foreach($rows as $row)
			{
					...
					{
						$dbname = $row->vehicle_dbhistory_name;
					}
	...
	function create_tables_archive($vdevice = "", $type="")
	{
		ini_set('memory_limit', '2G');
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		...
		foreach(array("gpsarchive") as $dbname)
		{
	...
	function create_tables_alert($vdevice = "", $type="")
	{
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		$this->db->select("vehicle_device");
		...
		$this->db->where("vehicle_user_id", 4408); //khusus indobara
		//$this->db->or_where("vehicle_user_id", 4576);
		$q = $this->db->get("vehicle");
		if ($q->num_rows() == 0) return;
		...
		foreach(array("gpshistory_alert") as $dbname)
		{
			$historydb = $this->load->database($dbname, TRUE);
			$i = 0;
			...
			foreach($rows as $row)
			{
				...
				$histtable = $this->load->view("db/gpsalert", FALSE, TRUE);			
				$sql = sprintf($histtable, strtolower($imei."_".$host)."_gps");						
				$historydb->query($sql);
				
				printf("=== %s_gps\n", strtolower($imei."_".$host));

				$histinfotable = $this->load->view("db/infoalert", FALSE, TRUE);
				$sql = sprintf($histinfotable, strtolower($imei."_".$host)."_info");			
				$historydb->query($sql);
	...
	function create_tables_command($vdevice = "", $type="")
	{
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		$this->db->select("vehicle_device");
		...
		foreach(array("gpshistory_command") as $dbname)
		{
			$historydb = $this->load->database($dbname, TRUE);
			$i = 0;
			...
			foreach($rows as $row)
			{
				...
				$histtable = $this->load->view("db/gpscommand", FALSE, TRUE);			
				$sql = sprintf($histtable, strtolower($imei."_".$host)."_gps");						
				$historydb->query($sql);
	...
	function create_tables_respon($vdevice = "", $type="")
	{
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		$this->db->select("vehicle_device");
		...
		foreach(array("gpshistory_respon") as $dbname)
		{
			$historydb = $this->load->database($dbname, TRUE);
			$i = 0;
			...
			foreach($rows as $row)
			{
				...
				$histtable = $this->load->view("db/gpsrespon", FALSE, TRUE);			
				$sql = sprintf($histtable, strtolower($imei."_".$host)."_gps");						
				$historydb->query($sql);
	...
	function create_tables_ritase($vdevice = "", $type="")
	{
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		$this->db->select("vehicle_device");
		...
		foreach(array("gpshistory_ritase") as $dbname)
		{
			$historydb = $this->load->database($dbname, TRUE);
			$i = 0;
			...
			foreach($rows as $row)
			{
				...
				$histtable = $this->load->view("db/gpsritase", FALSE, TRUE);			
				$sql = sprintf($histtable, strtolower($imei."_".$host)."_gps");						
				$historydb->query($sql);
	...
	function create_tables_radius($vdevice = "", $type="")
	{
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		$this->db->select("vehicle_device");
		...
		foreach(array("gpshistory_radius") as $dbname)
		{
			$historydb = $this->load->database($dbname, TRUE);
			$i = 0;
			...
			foreach($rows as $row)
			{
				...
				$histtable = $this->load->view("db/gpsradius", FALSE, TRUE);
				$sql = sprintf($histtable, strtolower($imei."_".$host)."_gps");
				$historydb->query($sql);
	...
	function dodaily($name="", $host="", $maxdata=9000, $offset = 0)
	{
		...
		$this->db->where("vehicle_user_id <>", "752");//csa
		$this->db->where("vehicle_user_id <>", "1032");//balrich
		$this->db->where("vehicle_user_id <>", "2284");//cals
		$this->db->where("vehicle_user_id <>", "2001");//atria
		$this->db->where("vehicle_user_id <>", "3388");//plant cibitung
		$this->db->where("vehicle_user_id <>", "3389");//plant cikande
		$this->db->where("vehicle_user_id <>", "3385");//plant cikarang
		$this->db->where("vehicle_user_id <>", "3395");//plant makassar
		$this->db->where("vehicle_user_id <>", "3394");//plant medan
		$this->db->where("vehicle_user_id <>", "3392");//plant palembang
		$this->db->where("vehicle_user_id <>", "3393");//plant pasuruan
		$this->db->where("vehicle_user_id <>", "3390");//plant purwakarta
		$this->db->where("vehicle_user_id <>", "3391");//plant semarang
		$this->db->where("vehicle_user_id <>", "3495");//kiani
		$this->db->where("vehicle_user_id <>", "3499");//radja
		$this->db->where("vehicle_user_id <>", "3606");//cas
		$this->db->where("vehicle_user_id <>", "4203");//bib
		$this->db->where("vehicle_user_id <>", "2974");//saro roti
		$this->db->where("vehicle_user_id <>", "4333");//demo fms
		$this->db->where("vehicle_user_id <>", "4408");//temanindobara
		
		$this->db->where("vehicle_device <>", "69969039633231@TK510");//demotk510bib
		
		$this->db->where("vehicle_type <>", "T6");
		$this->db->where("vehicle_type <>", "T8");
		$this->db->where("vehicle_type <>", "T8_2");
		$this->db->where("vehicle_type <>", "NDROID");
...
";
	...
	function archive($maxdata=1000)
	{
		...
		$cron_name = "MOVE TO ARCHIVE";
		$message =  urlencode(
					"".$cron_name." \n".
					
					"Total Vehicle: ".$i." of ".$totvehicle." \n".
					"Start: ".$starttime." \n".
					"Finish: ".$finishtime." \n"
					);
					
		$sendtelegram = $this->telegram_direct("-627628862",$message); //FMS CRON DAILY
...
";
	...
	function daily_user($user="", $order="asc", $maxdata=10000, $offset=0)
	{
		ini_set('memory_limit', '3G');
		...
		$this->db->where("vehicle_status <>", 3);
		//$this->db->where("vehicle_device <>", "002100000583@T5");//tes
		$this->db->select("vehicle_type, vehicle_device, vehicle_info");
		$q = $this->db->get("vehicle");
		if ($q->num_rows() == 0) return;
		...
		$sendtelegram = $this->telegram_direct("-993884493",$message);
		printf("===SENT TELEGRAM OK\r\n");
	}
	
	function daily_db($dbname="", $histdb="gpshistory", $maxdata=500, $offset=0)
	{
...
";
	...
	function dodaily_asc($name="", $host="", $maxdata=9000, $offset = 0)
	{
		ini_set('memory_limit', '3G');
		$start_time = date("d-m-Y H:i:s");
		...
		$this->db->where("vehicle_user_id <>", "3606");//cas
		$this->db->where("vehicle_user_id <>", "4203");//bib
		$this->db->where("vehicle_user_id <>", "974");//sari roti
		$this->db->where("vehicle_user_id <>", "2974");//sariroti
		$this->db->where("vehicle_user_id <>", "4333");//sariroti
		$this->db->where("vehicle_user_id <>", "4408");//temanindobara
...
";
	...
	function daily_vehicle($vehicledevice = "", $vehicletype = "", $maxdata=1000, $offset=0)
	{
		$this->db->distinct();
	...
	function dailydb_optimize($groupname="",$timezone="",$maxdata=1000)
	{
		$userid = "";
		$starttime = date("Y-m-d H:i:s");
		$start = mktime();
		...
		$totalport = count($rowsport);
		printf("Total Port : %s \r\n",$totalport);
			for ($i=0;$i<count($rowsport);$i++)
			{
				$table = "webtracking_gps";
				$tableinfo = "webtracking_gps_info";
				$port = $rowsport[$i]->port_value;
				$portdb = $rowsport[$i]->port_config;
						
				$this->dbport = $this->load->database($portdb, TRUE);							
			
				$this->dbport->query("OPTIMIZE TABLE GPS ".$table."");
				$this->dbport->query("OPTIMIZE TABLE INFO ".$tableinfo."");
				$this->dbport->close();
				$this->dbport->cache_delete_all();
				
			}
			
		
		$finishtime = date("Y-m-d H:i:s");
		$cron_name = "OPTIMIZE DB"." ".$groupname;
		$message =  urlencode(
					"".$cron_name." \n".
					"Start: ".$starttime." \n".
					"Finish: ".$finishtime." \n"
	...
	function dailydb_multiport_new($groupname="",$timezone="",$maxdata=1000)
	{
				...
				foreach($rows as $row)
				{
					...
					if ($qdatav->num_rows() > 0){
						...
						while(1)
						{
							...
							$configportdb = $this->load->database($portdb, TRUE);					
							$configportdb->limit($maxdata, $offset);
							$configportdb->select("gps_id,gps_name,gps_host,gps_type,gps_utc_coord,gps_status,
												   gps_latitude,gps_ns,gps_longitude,gps_ew,gps_speed,gps_course,gps_utc_date,
												   gps_mvd,gps_mv,gps_cs,gps_msg_ori,gps_time,gps_latitude_real,gps_longitude_real,gps_odometer,gps_workhour");
							$configportdb->where("gps_name", $devices[0]);
							$configportdb->where("gps_host", $devices[1]);
							$configportdb->where("gps_time <=", date("Y-m-d H:i:s", $now));
							$q = $configportdb->get($table);
		...
		$sendtelegram = $this->telegram_direct("-653753878",$message); //FMS cron report
		printf("===SENT TELEGRAM OK\r\n");
	...
	function dailydb_port_new($port="",$maxdata=1000)
	{
		...
		for($z=0;$z<count($rowsport);$z++)
		{
			...
			//looping vehicle
				...
				$sendtelegram = $this->telegram_direct("-993884493",$message); //FMS cron DAILY
				printf("===SENT TELEGRAM OK\r\n");
	...
	function dailydb_port_new2($port="",$maxdata=1000)
	{
		$userid = "";
		$starttime = date("Y-m-d H:i:s");
		$start = mktime();
		...
		for($z=0;$z<count($rowsport);$z++)
		{
			...
			//looping vehicle
				...
				{
					$vehicle_device_string = $rows[$y]->gps_name."@".$rows[$y]->gps_host;
					$devices = explode("@", $vehicle_device_string);
	...
	function daily_alert($sort="", $name="", $host="",$maxdata=5000, $offset = 0)
	{
		ini_set('memory_limit', '2G');
		$start_time = date("d-m-Y H:i:s");
		...
		$this->db->select("vehicle_type, vehicle_device, vehicle_dbname_live");
		if ($name != "" && $host != "")
		{
			$vedev = $name."@".$host;
			$this->db->where("vehicle_device", $vedev);
		...
		$this->db->where("vehicle_dbname_live !=", "");
		//$this->db->where("vehicle_status <>",3);
		$q = $this->db->get("vehicle");
		if ($q->num_rows() == 0) return;
		...
		foreach($rows as $row)
		{
			...
			unset($repairs);
			printf("history alert for %s (%d/%d)\n", $row->vehicle_device, ++$i, $totalvehicle);
			
			if (isset($row->vehicle_dbname_live) || !isset($row->vehicle_dbname_live))
			{		
					$database = $row->vehicle_dbname_live;
					$table = "webtracking_gps_alert";
					$tableinfo = "webtracking_gps_info_alert";
					...
					while(1)
					{
						...
						printf("=== jumlah data alert: %d, lama: %ds \n", $q->num_rows(), mktime()-$start);
				
						if ($q->num_rows() == 0) break;
				
						$historydb = $this->load->database("gpshistory_alert", TRUE);
						...
						foreach($gpses as $gps)
						{
							...
							$historydb->insert(strtolower($imei."_".$host)."_gps", $gps);
							$isdelete = true;
						}
					...
					if($host == "T5"){
						...
						while(1)
						{
							...
							foreach($gpses as $gps)
							{
								...
								$historydb->insert(strtolower($imei."_".$host)."_info", $gps);
								$isdelete = true;
							}
	...
	function repairtablehist($vdevice = "", $type="")
	{
		$this->db->distinct();
		$this->db->order_by("vehicle_id","desc");
		$this->db->select("vehicle_device,vehicle_dbhistory_name");
		...
		foreach(array("gpshistory") as $dbname)
		{
			...
			foreach($rows as $row)
			{
				printf("%d/%d Repair table %s for %s\n", ++$i, $total, $dbname, $row->vehicle_device);

				if($dbname == "gpshistory")
				{
					$dbname = $this->config->item("dbhistory_default");
					if($this->config->item("is_dbhistory") == 1)
					{
				...
				$histtable = $this->load->view("db/table", FALSE, TRUE);			
				$sql = sprintf($histtable, strtolower($row->vehicle_device)."_gps");						
				$historydb->query($sql);
				
				printf("=== %s_gps\n", strtolower($row->vehicle_device));

				$histinfotable = $this->load->view("db/table", FALSE, TRUE);
				$sql = sprintf($histinfotable, strtolower($row->vehicle_device)."_info");			
				$historydb->query($sql);
	...
	function clear_multiport($servername="")
	{
		$userid = "";
		$type_list = array("TOPTEN");
		$starttime = date("Y-m-d H:i:s");
		$start = mktime();
		...
		$this->db->where("config_group_database",$servername);
		$this->db->where("config_port_value !=","");
		//$this->db->where_in("config_device_type",$type_list);
		//$this->db->limit(3);
		$qport = $this->db->get("config_port");
		if ($qport->num_rows() == 0){
			printf("Tidak ada config \r\n");
		}else{
			$rowsport = $qport->result();
			$totalport = count($rowsport);
			printf("Total Port : %s \r\n",$totalport);
			...
			//looping vehicle table gps
			printf("FROM TABLE GPS \r\n");
			for($z=0;$z<count($rowsport);$z++)
			{
				$port = $rowsport[$z]->config_port_value;
				$portdb = $rowsport[$z]->config_port_database;
				
				//master nya dari table gps
				$this->db = $this->load->database($portdb,true);
				$this->db->group_by("gps_name");
				$this->db->select("gps_name,gps_host");
				$q = $this->db->get("gps");
				if ($q->num_rows() == 0){
					printf("Tidak ada Data : %s \r\n",$port);
				}
				else
				{
					$rows = $q->result();
					...
					//looping vehicle
						...
						foreach($rows as $row)
						{
							...
							$this->db = $this->load->database("default",true);
							$this->db->select("vehicle_id,vehicle_device,vehicle_dbhistory_name");
							$this->db->where("vehicle_device",$vehicle_device_string);
							$qdatav = $this->db->get("vehicle");
							$rowdatav = $qdatav->row();
							
							printf("[%s] Select Data %s (%d/%d) - GPS %s (%s/%s) \r\n", date("Y-m-d H:i:s"), $vehicle_device_string, ++$i, $totvehicle, $port, $z+1, $totalport); 
							//jika di webtracking master ada maka diskip
							if ($qdatav->num_rows() > 0){
								
								printf("==X SKIP ID Terdaftar di Sistem \r\n");
							}
							else
							{
								
								//jika tidak terdaftar maka dihapus 
								$table = "webtracking_gps";
								$tableinfo = "webtracking_gps_info";
								...
								if ($isdelete)
								{
									...
									$configportdb->where("gps_name", $devices[0]);
									$configportdb->where("gps_host", $devices[1]);
									$configportdb->where("gps_time <=", $now);
									$configportdb->delete($table);			
									
									printf("==DELETE GPS ID : %s Success \r\n", $vehicle_device_string);
								}
								...
								if ($isdelete)
								{
									...
									$configportdb = $this->load->database($portdb, TRUE);
									$configportdb->where("gps_info_device", $vehicle_device_string);
									$configportdb->where("gps_info_time <=", $now);
									$configportdb->delete($tableinfo);			

									printf("==DELETE INFO ID : %s Success \r\n", $vehicle_device_string);
								}
				...
				//DB LIVE ONLY
				if($rowsport[$z]->config_live_status == 1){
					$livedb = $rowsport[$z]->config_live_database;
					
					$this->db = $this->load->database($livedb,true);
					$this->db->group_by("gps_name");
					$this->db->select("gps_name,gps_host");
					$qlive = $this->db->get("gps");
					if ($qlive->num_rows() == 0){
						printf("Tidak ada Data Live : %s \r\n",$port);
					}
					else
					{
						$rows = $qlive->result();
						...
						//looping vehicle
							...
							foreach($rows as $row)
							{
								...
								printf("[%s] Select Data Live %s (%d/%d) - GPS %s (%s/%s) \r\n", date("Y-m-d H:i:s"), $vehicle_device_string, ++$i, $totvehicle, $port, $z+1, $totalport); 
								//jika di webtracking master ada maka diskip
								if ($qdatav->num_rows() > 0){
								...
								else
								{
									...
									if ($isdelete)
									{
										...
										$configportdb = $this->load->database($livedb, TRUE);							
										$configportdb->where("gps_name", $devices[0]);
										$configportdb->where("gps_host", $devices[1]);
										$configportdb->where("gps_time <=", $now);
										$configportdb->delete($table);			
										
										printf("==DELETE GPS ID LIVE : %s Success \r\n", $vehicle_device_string);
									}
									...
									if ($isdelete)
									{
										...
										printf("==DELETE INFO ID LIVE : %s Success \r\n", $vehicle_device_string);
									}
			...
			//looping vehicle table info
			printf("FROM TABLE INFO \r\n");
			for($z=0;$z<count($rowsport);$z++)
			{
				$port = $rowsport[$z]->config_port_value;
				$portdb = $rowsport[$z]->config_port_database;
				...
				//master nya dari table gps
				$this->db = $this->load->database($portdb,true);
				$this->db->group_by("gps_info_device");
				$this->db->select("gps_info_device");
				...
				else
				{
					...
					foreach($rows as $row)
					{
						$vehicle_device_string = $row->gps_info_device;
						$devices = explode("@", $vehicle_device_string);
						...
						printf("[%s] Select Data %s (%d/%d) - INFO %s (%s/%s) \r\n", date("Y-m-d H:i:s"), $vehicle_device_string, ++$i, $totvehicle, $port, $z+1, $totalport); 
						//jika di webtracking master ada maka diskip
						if ($qdatav->num_rows() > 0){
							
							printf("==X SKIP ID Terdaftar di Sistem %d \r\n", $vehicle_device_string);
						}
						else
						{
				...
				if($rowsport[$z]->config_live_status == 1){
					...
					else
					{
						...
						foreach($rows as $row)
						{
							...
							printf("[%s] Select Data %s (%d/%d) - INFO LIVE %s (%s/%s) \r\n", date("Y-m-d H:i:s"), $vehicle_device_string, ++$i, $totvehicle, $port, $z+1, $totalport); 
							//jika di webtracking master ada maka diskip
							if ($qdatav->num_rows() > 0){
							...
							else
							{
								...
								if ($isdelete)
								{
									...
									printf("==DELETE GPS LIVE ID : %s Success \r\n", $vehicle_device_string);
								}
								...
								if ($isdelete)
								{
									...
									printf("==DELETE INFO LIVE ID : %s Success \r\n", $vehicle_device_string);
								}
		...
		$finishtime = date("Y-m-d H:i:s");
		$cron_name = "CLEAR DB MULTIPORT"." ".$servername;
		$message =  urlencode(
					"".$cron_name." \n".
					"Start: ".$starttime." \n".
					"Finish: ".$finishtime." \n"
	...
	function daily_alert_dblive($dblive="",$maxdata=10000, $offset = 0)
	{
		ini_set('memory_limit', '2G');
		$start_time = date("Y-m-d H:i:s");
		...
		$this->db->distinct();
		$this->db->order_by("vehicle_id","asc");
		$this->db->where("vehicle_dbname_live", $dblive);
		...
		foreach($rows as $row)
		{
			...
			{		
					...
					if (isset($isdelete))
					{
						...
						$this->dblive->where("gps_name", $devices[0]);
						$this->dblive->where("gps_host", $devices[1]);
						//$this->dblive->where("gps_time <=", date("Y-m-d H:i:s", $now));
						$this->dblive->where("gps_time <=", $now);
						$this->dblive->delete($table);
					...
					if($host == "T5"){
						...
						while(1)
						{
							...
							$this->db->where("gps_info_device", $row->vehicle_device);
							//$this->db->where("gps_info_time <=", date("Y-m-d H:i:s", $now));
							$this->db->where("gps_info_time <=", $now);
							$q = $this->db->get($tableinfo);					
							$total = $q->num_rows();
						...
						if (isset($isdelete))
						{
							printf("=== delete old data\r\n");
							$this->dblive = $this->load->database($dblive, TRUE);
							$this->dblive->where("gps_info_device", $row->vehicle_device);
							//$this->dblive->where("gps_info_time <=", date("Y-m-d H:i:s", $now));
							$this->dblive->where("gps_info_time <=", $now);
							$this->dblive->delete($tableinfo);					
		...
		$finishtime = date("Y-m-d H:i:s");
		$cron_name = "DAILY ALERT: ".$dblive;
		$message =  urlencode(
					"".$cron_name." \n".
					"Vehicle: ".$no_urut." of ".$totalvehicle." \n".
					"Start: ".$start_time." \n".
					"Finish: ".$finishtime." \n"
					);

		$sendtelegram = $this->telegram_direct("-627628862",$message); //FMS Testing
		printf("===SENT TELEGRAM OK\r\n");
	...
	function daily_command_dblive($dblive="",$maxdata=10000, $offset = 0)
	{
		ini_set('memory_limit', '2G');
		$start_time = date("Y-m-d H:i:s");
		...
		foreach($rows as $row)
		{
			...
			unset($repairs);
			printf("history Command for %s (%d/%d)\n", $row->vehicle_device, ++$i, $totalvehicle);
			
			if (isset($row->vehicle_dbname_live) || !isset($row->vehicle_dbname_live))
			{		
					$database = $row->vehicle_dbname_live;
					$table = "webtracking_command";
					...
					{
						$this->db->limit($maxdata, $offset);
						$this->db->where("command_device", $devices[0]);
						//$this->db->where("command_date <=", date("Y-m-d H:i:s", $now));
						$this->db->where("command_date <=", $now);
						$q = $this->db->get($table);					
						$total = $q->num_rows();
							
						printf("=== jumlah data command: %d, lama: %ds \n", $q->num_rows(), mktime()-$start);
				
						if ($q->num_rows() == 0) break;
				
						$historydb = $this->load->database("gpshistory_command", TRUE);
				
						$gpses = $q->result_array();
						$q->free_result();
						foreach($gpses as $gps)
						{
							unset($gps['command_id']);
					...
					if (isset($isdelete))
					{
						printf("=== delete old data\r\n");
						$this->dblive = $this->load->database($dblive, TRUE);
						$this->dblive->where("command_device", $devices[0]);
						//$this->dblive->where("command_date <=", date("Y-m-d H:i:s", $now));
						$this->dblive->where("command_date <=", $now);
						$this->dblive->delete($table);
		...
		$finishtime = date("Y-m-d H:i:s");
		$cron_name = "DAILY COMMAND: ".$dblive;
		$message =  urlencode(
					"".$cron_name." \n".
					"Vehicle: ".$no_urut." of ".$totalvehicle." \n".
					"Start: ".$start_time." \n".
	...
	function daily_ritase_dblive($dblive="",$maxdata=10000, $offset = 0)
	{
		ini_set('memory_limit', '2G');
		$start_time = date("Y-m-d H:i:s");
		...
		foreach($rows as $row)
		{
			...
			unset($repairs);
			printf("history Ritase for %s (%d/%d)\n", $row->vehicle_device, ++$i, $totalvehicle);
			
			if (isset($row->vehicle_dbname_live) || !isset($row->vehicle_dbname_live))
			{		
					$database = $row->vehicle_dbname_live;
					$table = "webtracking_ritase";
					
					$start = mktime();
					//$now = mktime(0, 0, 0, date('n'), date('j', $start), date('Y'))-7*3600;
					$now_time = date("Y-m-d H:i:s");
					$yesterday_time = date("Y-m-d H:i:s",strtotime($now_time . "-1days"));
					$now_first_hour = date("Y-m-d 23:59:59", strtotime($yesterday_time));
					$now = date("Y-m-d H:i:s",strtotime($now_first_hour . "-7hours"));
					//print_r($now." | ".$now_first_hour." | ".$now_time);exit();
					...
					{
						$this->db->limit($maxdata, $offset);
						$this->db->where("ritase_device", $devices[0]);
						//$this->db->where("ritase_gpstime <=", date("Y-m-d H:i:s", $now));
						$this->db->where("ritase_gpstime <=", $now);
						
						$q = $this->db->get($table);					
						$total = $q->num_rows();
							
						printf("=== jumlah data ritase: %d, lama: %ds \n", $q->num_rows(), mktime()-$start);
				
						if ($q->num_rows() == 0) break;
				
						$historydb = $this->load->database("gpshistory_ritase", TRUE);
					...
					if (isset($isdelete))
					{
						printf("=== delete old data\r\n");
						$this->dblive = $this->load->database($dblive, TRUE);
						$this->dblive->where("ritase_device", $devices[0]);
						//$this->dblive->where("ritase_gpstime <=", date("Y-m-d H:i:s", $now));
						$this->dblive->where("ritase_gpstime <=", $now);
						$this->dblive->delete($table);
	...
	function daily_respon_dblive($dblive="",$maxdata=10000, $offset = 0)
	{
		ini_set('memory_limit', '2G');
		$start_time = date("Y-m-d H:i:s");
		...
		foreach($rows as $row)
		{
			...
			unset($repairs);
			printf("history Respon for %s (%d/%d)\n", $row->vehicle_device, ++$i, $totalvehicle);
			
			if (isset($row->vehicle_dbname_live) || !isset($row->vehicle_dbname_live))
			{		
					$database = $row->vehicle_dbname_live;
					$table = "webtracking_respon";
					...
					{
						$this->db->limit($maxdata, $offset);
						$this->db->where("respon_device", $devices[0]);
						//$this->db->where("respon_date <=", date("Y-m-d H:i:s", $now));
						$this->db->where("respon_date <=", $now);
						$q = $this->db->get($table);					
						$total = $q->num_rows();
							
						printf("=== jumlah data respon: %d, lama: %ds \n", $q->num_rows(), mktime()-$start);
				
						if ($q->num_rows() == 0) break;
				
						$historydb = $this->load->database("gpshistory_respon", TRUE);
					...
					if (isset($isdelete))
					{
						printf("=== delete old data\r\n");
						$this->dblive = $this->load->database($dblive, TRUE);
						$this->dblive->where("respon_device", $devices[0]);
						//$this->dblive->where("respon_date <=", date("Y-m-d H:i:s", $now));
						$this->dblive->where("respon_date <=", $now);
						$this->dblive->delete($table);
	...
	function daily_radius_dblive($dblive="",$maxdata=10000, $offset = 0)
	{
		ini_set('memory_limit', '2G');
		$start_time = date("Y-m-d H:i:s");
		...
		foreach($rows as $row)
		{
			...
			{		
					...
					{
						$this->db->limit($maxdata, $offset);
						$this->db->where("radius_host", $devices[0]);
						$this->db->where("radius_host_time <=", $now);
						
						$q = $this->db->get($table);					
						$total = $q->num_rows();
							
						printf("=== jumlah data Radius: %d, lama: %ds \n", $q->num_rows(), mktime()-$start);
				
						if ($q->num_rows() == 0) break;
				
						$historydb = $this->load->database("gpshistory_radius", TRUE);
					...
					if (isset($isdelete))
					{
						printf("=== delete old data\r\n");
						$this->dblive = $this->load->database($dblive, TRUE);
						$this->dblive->where("radius_host", $devices[0]);
						$this->dblive->where("radius_host_time <=", $now);
						$this->dblive->delete($table);
	...
	function dailydb_multiport_new_2($groupname="",$timezone="",$maxdata=5000)
	{
		$userid = "";
		$starttime = date("Y-m-d H:i:s");
		$start = mktime();
	...
	function dailydb_multiport_new_3($groupname="",$timezone="",$maxdata=10000)
	{
		ini_set('memory_limit', '2G');
		$userid = "";
		$starttime = date("Y-m-d H:i:s");
