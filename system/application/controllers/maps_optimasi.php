...
function heatmap2(){
	...
	$companyid               = $this->sess->user_company;
	$user_dblive             = $this->sess->user_dblive;
	$masterAllVehicle        = $this->m_poipoolmaster->getmastervehicle(0);
	$vehicleAutochecknotnull = $this->m_poipoolmaster->getmastervehicle(1);

	// echo "<pre>";
	// var_dump($vehicleDbLive);die();
	// echo "<pre>";

	$datafix            = array();

	for ($i=0; $i < sizeof($vehicleAutochecknotnull); $i++) {
		$device = explode("@", $vehicleAutochecknotnull[$i]['vehicle_device']);
		$device0 = $device[0];
		$device1 = $device[1];

		$getdata                 = $this->m_poipoolmaster->getLastPosition("webtracking_gps", $user_dblive, $device0);
			if (sizeof($getdata) > 0) {
				$jsonnya = json_decode($getdata[0]['vehicle_autocheck'], true);
						array_push($datafix, array(
						 "vehicle_id"           => $vehicleAutochecknotnull[$i]['vehicle_id'],
 						 "vehicle_user_id"      => $vehicleAutochecknotnull[$i]['vehicle_user_id'],
 						 "vehicle_device"       => $vehicleAutochecknotnull[$i]['vehicle_device'],
 						 "vehicle_no"           => $vehicleAutochecknotnull[$i]['vehicle_no'],
 						 "vehicle_name"         => $vehicleAutochecknotnull[$i]['vehicle_name'],
 						 "vehicle_active_date2" => $vehicleAutochecknotnull[$i]['vehicle_active_date2'],
 						 "vehicle_card_no"      => $vehicleAutochecknotnull[$i]['vehicle_card_no'],
 						 "vehicle_type"         => $vehicleAutochecknotnull[$i]['vehicle_type'],
 						 "vehicle_company"      => $vehicleAutochecknotnull[$i]['vehicle_company'],
 						 "vehicle_subcompany"   => $vehicleAutochecknotnull[$i]['vehicle_subcompany'],
 						 "vehicle_group"        => $vehicleAutochecknotnull[$i]['vehicle_group'],
 						 "vehicle_subgroup"     => $vehicleAutochecknotnull[$i]['vehicle_subgroup'],
						 "auto_last_lat"        => substr($jsonnya['auto_last_lat'], 0, 10),
			 			 "auto_last_long"       => substr($jsonnya['auto_last_long'], 0, 10)
						));
			}
	}
	...
	$this->params["sidebar"]        = $this->load->view('newdashboard/partial/sidebar', $this->params, true);
	$this->params["chatsidebar"]    = $this->load->view('newdashboard/partial/chatsidebar', $this->params, true);
	$this->params["content"]        = $this->load->view('newdashboard/trackers/maps_monitoring', $this->params, true);
	$this->load->view("newdashboard/partial/template_dashboard_new", $this->params);
}
...
function mapsdata(){
	...
	echo json_encode(array("datamaps" => $datafix));
	// echo "<pre>";
	// var_dump($datafix);die();
	// echo "<pre>";
