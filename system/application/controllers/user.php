...
class User extends Base {
	...
	function savepass($id)
	{
		...
		$firsttime = isset($_POST['firsttime']) ? trim($_POST['firsttime']): ""; 
		
		//khusus driver 
		if($this->sess->user_id_role == 9){
			if(!is_numeric($pass)) {
				$callback['error']   = true;
				$callback['message'] = "Password/PIN hanya boleh diinput 6 digit angka";
			...
			if(strlen($pass) < 12) {
				$callback['error']   = true;
				$callback['message'] = "Password harus mengandung minimal 12 Digit";
		...
		// {
			...
			// }else{
				// $sql = "SELECT * FROM ".$this->db->dbprefix."user WHERE user_id = '".$id."'";
			// }
			// $q = $this->db->query($sql);
		...
		$sql = "UPDATE ".$this->db->dbprefix."user SET user_password_default=1, user_pass = PASSWORD('".$pass."') WHERE user_id = '".$id."'";
		if($firsttime == ""){
			$callback['message'] = $this->lang->line('lchangepassword_success');
		}else{
			$callback['message'] = "Password has been changed. please login again!";
		}
