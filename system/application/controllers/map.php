...
class Map extends Base {
	...
	function lastinfonew()
	{
		...
		if ($vehiclemv03 != "0000") {
			...
			$iddriverchange 									 = 631; // ID DRIVER CHANGE LEVEL 2
			$getdriverchangeinsecurityevidence = $this->fromsecurityevidence($dbtable, $vehiclemv03, $iddriverchange);
			if (sizeof($getdriverchangeinsecurityevidence) > 0) {
				$datefromsecurityevidence          = date("Y-m-d H:i:s", strtotime($getdriverchangeinsecurityevidence[0]['alarm_report_start_time'])+60*60);
				// $vehiclemv03.'||'.$changedrivertime.'||'.$datefromsecurityevidence.'-before : '.$getdriverchangeinsecurityevidence[0]['alarm_report_start_time']
				// NONE DRIVER
				if ($datefromsecurityevidence > $changedrivertime) {
	...
	function fromsecurityevidence($dbtable, $driverchangeid, $idtype){
		// echo "<pre>";
		// var_dump($dbtable.'-'.$driverchangeid.'-'.$idtype);die();
		// echo "<pre>";
		$this->dbtensor = $this->load->database("tensor_report", true);
		$this->dbtensor->select("alarm_report_start_time");
		$this->dbtensor->where("alarm_report_imei", $driverchangeid);
		$this->dbtensor->where("alarm_report_type", $idtype);
		$this->dbtensor->order_by("alarm_report_start_time", "DESC");
		$this->dbtensor->limit(1);
		return $this->dbtensor->get($dbtable)->result_array();
	}
	...
	function maps_hrm(){
      ...
      $this->sess = $q->row();
		...
		$companyid      = $_POST['companyid'];
		$forclearmaps   = $this->m_poipoolmaster->getmastervehicleHRM();
		$datavehicleall = $this->m_poipoolmaster->getmastervehicleHRM();
		$datafix        = array();
		$dataexpired    = array();
	...
	function mapsstandard_hrm(){ // maps with overlay seperti di history map
		...
		$companyid 											 = $_POST['companyid'];
		$forclearmaps                    = $this->m_poipoolmaster->getmastervehicleHRM();
		$mastervehicle                   = $this->m_poipoolmaster->getmastervehicleHRM($companyid);
