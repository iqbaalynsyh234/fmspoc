...
class Bib_cronjob_frombm02 extends Base {

	function Bib_cronjob_frombm02()
	{
		parent::Base();
	...
	function getAllVehicle_mdvronly_tibprod($userid, $company){
		//GET DATA FROM DB
		$this->db     = $this->load->database("tibprod_default", true);
		$this->db->select("*");
		$this->db->order_by("vehicle_no","asc");
			if ($company != "all") {
	...
	function loginforcheckdsmadasison($username, $password){
		$url_login   = "http://gpsdvr.pilartech.co.id/808gps/StandardApiAction_login.action?account=".$username."&password=".$password;
		$_h = curl_init();
				...
				// {"":"P","auto_last_update":"2022-05-24 14:24:06","auto_last_check":"2022-05-24 00:00:00","auto_last_position":"EST ROAD  BANJAR KALIMANTAN SELATAN","auto_last_lat":"-3.502445","auto_last_long":"115.593888","auto_last_engine":"ON","auto_last_gpsstatus":"OK","auto_last_speed":"15","auto_last_course":"128","auto_last_road":"muatan","auto_last_hauling":"out","auto_last_rom_name":"ROM A1","auto_last_rom_time":"2022-05-22 08:53:55","auto_last_port_name":"PORT BIB","auto_last_port_time":"2022-05-24 12:44:37","auto_flag":0,"vehicle_gotohistory":0,"auto_change_engine_status":"ON","auto_change_engine_datetime":"2022-05-24 14:23:33","auto_change_position":"KM 19.5,  TANAH BUMBU KALIMANTAN SELATAN","auto_change_coordinate":"-3.57265,115.655123"}"
					...
					if ($isinhauling == "in") {
						...
						$deviceID = $masterdatavehicle[$i]->vehicle_mv03; //"819058587559" 142045031243;//$masterdatavehicle[$i]->vehicle_mv03; BKA
							if ($deviceID != 0000) {
								$datafromapi = file_get_contents("http://gpsdvr.pilartech.co.id/StandardApiAction_queryAlarmDetail.action"."?jsession=".$jsession."&devIdno=".$deviceID."&begintime=".$startdate."&endtime=".$enddate."&armType=".$alarmtypefix."&handle=0&currentPage=1&pageRecords=1000&toMap=1&checkend=0&updatetime=".$updatetime."&language=en");
									if ($datafromapi) {
										$createdurl = "http://gpsdvr.pilartech.co.id/StandardApiAction_queryAlarmDetail.action"."?jsession=".$jsession."&devIdno=".$deviceID."&begintime=".$startdate."&endtime=".$enddate."&armType=".$alarmtypefix."&handle=0&currentPage=1&pageRecords=5&toMap=1&checkend=0&updatetime=".$updatetime."&language=en";
	...
	function violationmonitoringhistorikal($userid){
		...
		$datafix           = array();
		$masterdatavehicle = $this->getAllVehicle_mdvronly_tibprod($userid, "all");
		$violationmaster   = $this->m_violation->getviolationmaster_tibprod();

		// GET ALARM MASTER
		$alarmtypefromaster = array();
			for ($j=0; $j < sizeof($violationmaster); $j++) {
				usleep(500);
				$alarmbymaster = $this->m_violation->getalarmbytype_tibprod($violationmaster[$j]['alarmmaster_id']);
				for ($k=0; $k < sizeof($alarmbymaster); $k++) {
					usleep(500);
					$alarmtypefromaster[] = $alarmbymaster[$k]['alarm_type'];
				}
		...
		// $time       = date("H:i:s", strtotime("-2hours"));
		$time          = date("H:i:s", strtotime($nowtime_wita . "-10 Minutes"));
		$startdate     = $date.'%20'.$time;
		...
		$totaldata_baru = 0;
										...
										if (isset($fromapidecode->alarms) != NULL) {
											...
											for ($j=0; $j < $totaldatacallback; $j++) {
												...
												if (in_array($fromapidecode->alarms[$j]->atp, $alarmtypefromaster)){
													...
													$dblivefix = "";
														if ($masterdatavehicle[$i]->vehicle_dbname_live == "webtracking_gps_temanindobara_live") {
															$dblivefix = "tibprod_webtracking_gps_temanindobara_live";
														}

													$geofence_location = $this->m_poipoolmaster->getGeofence_location_other_live($fromapidecode->alarms[$j]->smlng, $fromapidecode->alarms[$j]->smlat, $masterdatavehicle[$i]->vehicle_user_id, $dblivefix);
													// echo "<pre>";
													...
													if ($geofence_location) {
														...
														$positionalert     = $this->gpsmodel->GeoReverse_tibprod($latitude_data,$longitude_data);
														if ($positionalert->display_name != "Unknown Location!") {
															$positionexplode = explode(",", $positionalert->display_name);
															$position = $positionexplode[0];
														}else {
													...
													array_push($datafix, array(
														...
														"gps_alert"          => $atpStr,
														"gps_time"           => $fromapidecode->alarms[$j]->bTimeStr,
			...
			$message =  urlencode(
						"VIOLATION MONITORING HISTORIKAL FROM ABDIDALEM (TEMAN INDOBARA PRODUCTION) \n".
						"Total Data Dicheck: ".sizeof($masterdatavehicle)." \n".
	...
	function violationmonitoringhistorikal_perday($userid, $startdateparam=""){
		date_default_timezone_set("Asia/Jakarta");
		$cronstartdate = date("Y-m-d H:i:s");
		print_r("CRON START : ". $cronstartdate . "\r\n");
		$datatimeforevidence  = date("Y-m-d H:i:s", strtotime("-60 minutes"));
		...
		$masterdatavehicle = $this->getAllVehicle_mdvronly_tibprod($userid, 1959);
		$violationmaster   = $this->m_violation->getviolationmaster_tibprod();
		...
		if ($startdateparam == "") {
			...
			$date       = date("Y-m-d", strtotime($startdateparam));
			// $date       = date("Y-m-d");
			// $time    = date("H:i:s", strtotime("-2hours"));
			$time       = "00:00:00";
			$startdate  = $date.'%20'.$time;

			$edate      = date("Y-m-d", strtotime($startdateparam));
			// $edate      = date("Y-m-d");
			$etime      = "23:59:59";
			$enddate    = $edate.'%20'.$etime;
			$updatetime = "2020-07-23%2023:59:59";
		...
		$totaldata_baru = 0;
										...
										if (isset($fromapidecode->alarms) != NULL) {
											...
											for ($j=0; $j < $totaldatacallback; $j++) {
												...
												if (in_array($fromapidecode->alarms[$j]->atp, $alarmtypefromaster)){
													...
													$checkindb = $this->m_securityevidence->checktodbviolationtensor($dbtable, $masterdatavehicle[$i]->vehicle_device, $fromapidecode->alarms[$j]->bTimeStr);
													$company   = $this->dashboardmodel->getcompany_idforevidencetibprod($masterdatavehicle[$i]->vehicle_company);
			...
			$duration_sec = $end_1 - $start_1;
			$message =  urlencode(
						"VIOLATION MONITORING HISTORIKAL COLLECTING YESTERDAY DATA FROM ABDIDALEM (TEMAN INDOBARA PRODUCTION) \n".
						"Total Data Dicheck: ".sizeof($masterdatavehicle)." \n".
						"Total Data Baru: ".$totaldata_baru." \n".
						"Start: ".$cronstartdate." \n".
						"Finish: ".date("Y-m-d H:i:s")." \n".
	...
	function violationmonitoringhistorikal_skipcondition($userid){
		date_default_timezone_set("Asia/Jakarta");
		$cronstartdate = date("Y-m-d H:i:s");
		print_r("CRON START : ". $cronstartdate . "\r\n");
		$datatimeforevidence  = date("Y-m-d H:i:s", strtotime("-60 minutes"));
		...
		$edate      = date("Y-m-d");
		// $etime   = "23:59:59";
		$etime      = date("H:i:s", strtotime("+1 hours"));
		$enddate    = $edate.'%20'.$etime;
		$updatetime = "2020-07-23%2023:59:59";
		print_r("RANGE DATETIME SEARCH : ". $startdate ." s/d " . $enddate . "\r\n");
		...
		$totaldata_baru = 0;
			...
			$duration_sec = $end_1 - $start_1;
			$message =  urlencode(
						"VIOLATION MONITORING HISTORIKAL COLLECTING DATA PER 60 MENIT FROM ABDIDALEM (TEMAN INDOBARA PRODUCTION) \n".
						"Total Data Dicheck: ".sizeof($masterdatavehicle)." \n".
						"Total Data Baru: ".$totaldata_baru." \n".
						"Start: ".$cronstartdate." \n".
						"Finish: ".date("Y-m-d H:i:s")." \n".
	...
	function telegram_direct($groupid,$message)
		...
		function checkdsmadasison($userid){
			...
			$password                         = "000000";
			// $loginbaru                        = file_get_contents("http://gpsdvr.pilartech.co.id/StandardApiAction_login.action?account=".$username."&password=".$password);
			$loginbaru                        = file_get_contents("http://182.253.236.246/StandardApiAction_login.action?account=".$username."&password=".$password);
			$loginbarudecode                  = json_decode($loginbaru);
			$jsession                         = $loginbarudecode->jsession;
							...
							for ($j=0; $j < sizeof($masterdatavehicle); $j++) {
								...
								if ($company_id == $vehicle_company) {
									if ($vehiclemv03 != 0000) {
											...
											$dataonline = file_get_contents("http://182.253.236.246/StandardApiAction_getDeviceStatus.action?jsession=".$jsession."&devIdno=".$vehiclemv03."&toMap=1&driver=0&language=en");
												if ($dataonline) {
		...
		function checkdsmadasisontesting($userid){
			...
			$username                         = "temanindobara";
			$password                         = "000000";
			// $loginbaru                        = file_get_contents("http://gpsdvr.pilartech.co.id/808gps/StandardApiAction_login.action?account=".$username."&password=".$password);
			$loginbaru                        = file_get_contents("http://182.253.236.246/StandardApiAction_login.action?account=".$username."&password=".$password);
		...
		function violationtextmode($userid){
			...
			for ($i=0; $i < sizeof($masterviolation); $i++) {
				if (in_array($masterviolation[$i]['violation_position'], $street_onduty)) {
					$telegram_group_dt = $this->get_telegramgroup_overspeed($masterviolation[$i]['violation_vehicle_companyid']);
		...
		function violationhistorikalskipcondition($userid){
			...
			$totaldata_baru = $totaltelesend;
				...
				$duration_sec = $end_1 - $start_1;
				$message =  urlencode(
							"VIOLATION MONITORING HISTORIKAL SKIP CONDITION \n".
							"Total Data Dicheck: ".sizeof($masterdatavehicle)." \n".
