...
class Autorestart extends Base {
	...
	// alert Port to Port (tanpa masuk ROM) - with WA & extend ritase fraud + 10 menit
	...
	// alert dari ROM berhenti diluar geofence
	function autocheck_new2($groupname="", $userid="4408", $order="asc")
	{
		...
		$DT_list_company = $this->config->item('DT_company');
					...
					$gps = $this->getlastposition_fromDBLive($vehicle_imei,$vehicle_dblive);
						...
						if(count($gps)>0){
							...
							if(isset($lastposition)){
								...
								// NOT in STREET ON DUTY (HAULING, ROM, PORT)
								...
								if (in_array($vehiclecompany, $DT_list_company))
								{
									...
									{
										$fraud_anomali = 0;
										$limit_rom_time = 60*60; //default 30 menit 
											...
											$last_rom_date_new = date("Y-m-d", strtotime($lastrom_time_new));
												...
												else
												{
													...
													$message = urlencode(
														...
														if($fraud_anomali == 0)
														{
															$sendtelegram = $this->telegram_direct("-891281722",$message); //telegram BIB GEOFENCE ALERT
															$notif_wa_ritase_fraud = $this->sendnotif_wa_fraud_link($wa_token,$title_name,$gps_realtime,$rowvehicle[$i]->vehicle_no,$street_name,$url,$speed,$lastrom_text_new,$lastport_text_new,$attachmentlink);
	...
	function outofport_check($userid="",$orderby=""){
		...
		if(count($rows)>0){
			...
			$limit_sec = 3600+1300; //1jam -> 1.5 jam
			$limit_sec2 = 3600*2; //1jam 30 menit --> 2 jam */
