...
class Autorestart extends Base {
	...
	function autocheck_new_bk_2022_08_07($groupname="", $userid="4408", $order="asc")
	{
					...
					$gps = $this->getlastposition_fromDBLive($vehicle_imei,$vehicle_dblive);
						...
						if(count($gps)>0){
							...
							printf("===Engine %s \r\n", $engine);
										...
										$nonbib_register = array("SUNGAI DANAU","KINTAB","sungai danau","kintab","Non BIB Anzawara","Non BIB TBR/SDJ"
																);
	...
	function autocheck_new_bk_2022_08_23($groupname="", $userid="4408", $order="asc")
	{
		...
		for($x=0;$x<$totalmodem;$x++)
		{
			...
			$this->db->where("vehicle_modem", $modem);
			$this->db->where("vehicle_status <>", 3);
			//$this->db->where("vehicle_no", "BSL 2310"); // EST 1480
