...
class Esdm extends Base
{
	...
	function board()
	{
		...
		// $this->params["content"]        = $this->load->view('newdashboard/esdm/v_dashboard_esdm_mn', $this->params, true);
	...
	function search()
	{
		...
		if ($periode == "custom") {
			...
			$diff = strtotime(date("Y-m-d")) - strtotime($startdate);
			if ($diff < 0) {
				$err = true;
				$msg = "Date is not correct!";
			}
			...
			$edate = date("Y-m-d 23:59:59", strtotime($nowyear . "-" . $nowmonth . "-" . $nowday));
			// $sdate = date("Y-m-d H:i:s ", strtotime($nowyear . "-" . $nowmonth . "-" . $firstday . " " . "00:00:00"));
