...
class Fuelreport extends Base
{
    ...
    function index()
    {
        ...
        if ($privilegecode == 1) {
            ...
            $this->params["content"]        = $this->load->view('newdashboard/report/v_fuel_report', $this->params, true);
            ...
            if ($interval == 1) {
                ...
                for ($x = 0; $x < $total_loc; $x++) {
                    ...
                    if ($nosort == $total_loc) {
                        ...
                        $hour_only = date("H", strtotime($rows_loc[$x]->location_report_gps_time));
                        if (!isset($data_fix[$hour_only])) {
                            //data dari gps time, jam pertama
                            $data_fix[$hour_only] =  $rows_loc[$x]->location_report_fuel_data;
                            $prev = $hour_only - 1;
                            if ($x == 0) {
                ...
                for ($x = 0; $x < $total_loc; $x++) {
                    ...
                    if ($nosort == $total_loc) {
                        ...
                        $hour = date("H", strtotime($rows_loc[$x]->location_report_gps_time));
                        $minute = date("i", strtotime($rows_loc[$x]->location_report_gps_time));
                        ...
                        $data_fix[$i] =  array(
                            "hour" => $hour,
                            "minute" => $minute,
                            "fuel" => $rows_loc[$x]->location_report_fuel_data
                        );
                        $prev = $i - 1;
                        if ($prev < 0) {
                            //lewatkan pengecekan di data pertama dan simpan juga ke data terakhir
    ...
    function getfuelQuery_old($vehicle, $startdate, $enddate, $dbtable_location)
    {
