          ...
          <div class="row">
            ...
            <div class="col-md-4" id="realtimealertshowhide">
              ...
              <div class="panel" id="panel_form">
                <header class="panel-heading" id="realtime">
                  Realtime Alert
                  Show :
...
<div id="modalState" class="modal">
  <div class="modal-content-state">
    ...
    <button type="button" name="button" id="export_xcel_info" class="btn btn-warning btn-sm">Export Excel</button>
...
$(document).ready(function() {
  ...
  jQuery("#export_xcel_info").click(function()
  {
    ...
    // window.open('data:application/vnd.ms-excel,' + encodeURIComponent(jQuery('#divexportofflinevehicle').html()));
    var isi = $('#modalStateContent').html();
    $("#divtoUpload").html(isi);
    // $("#divtoUpload .attachment2").html("link video");
    var myBlob = new Blob([$("#modalStateContent").html()], {
    ...
    a.href = url;
    a.download = "Offline Vehicle" + ".xls";
    a.click();
    setTimeout(function() {
  ...
  $.post("<?php echo base_url() ?>securityevidence/realtimealertnew", {intervalke : intervalke, lasttimealert : lasttimealert, limitalert : limitalert}, function(response){
    ...
    if (response.code == 200) {
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        console.log("gps_speed : ", gps_speed);
        console.log("gps_speed_limit : ", gps_speed_limit);
...
function updateinfowindow(nopol){
  var deviceid = nopol;
    if (deviceid == 0) {
      ...
      $.post("<?php echo base_url() ?>maps/forsearchvehicle", data, function(response){
        jQuery("#loader2").hide();
        console.log("ini respon updateinfowindow bang: ", response);
        if (response.code == 400) {
          alert("Data tidak ditemukan");
        }else {
          infowindowkedua      = new google.maps.InfoWindow();
...
function showTableMuatan(){
  ...
  $.post("<?php echo base_url() ?>maps/km_quickcount_newadminonly", {companyid : companyid}, function(response){
    jQuery("#loader2").hide();
        console.log("response km_quickcount_newadminonly : ", response);
...
function km_quickcount_simultan(){
  ...
  $.post("<?php echo base_url() ?>maps/km_quickcount_newadminonly", {companyid : companyid}, function(response){
    jQuery("#loader2").hide();
    console.log("response km_quickcount_newadminonly simultan : ", response);
    var datamuatan         = response.dataMuatan;
    var datakosongan       = response.dataKosongan;
    var datafixlimit  = response.datafixlimit;
    var arraydatamuatan    = Object.keys(datamuatan).map((key) => [String(key), datamuatan[key]]);
...
function showTableRom(){
  ...
  jQuery("#loader2").show();
  $.post("<?php echo base_url() ?>maps/getstreetautomaticbycompanyidadminonly", {typeofstreet:3, companyid : companyid}, function(response){
    jQuery("#loader2").hide();
      ...
      if (datafix.length > 0) {
        ...
        for (var i = 0; i < datafix.length; i++) {
          ...
          var lastenginefix;
            ...
            var autolaststatus = datafix[i].auto_last_status;
              if (autolaststatus == "K") {
                htmlpool += '<tr style="background-color: yellow;">';
              }else if (autolaststatus == "M") {
                htmlpool += '<tr style="background-color: red;">';
              }else {
                htmlpool += '<tr style="background-color: white;">';
              }
...
function showTablePort(){
  ...
  var companyid = $("#contractor").val();
  jQuery("#loader2").show();
  $.post("<?php echo base_url() ?>maps/getstreetautomaticbycompanyidadminonly", {typeofstreet:4, companyid : companyid}, function(response){
       ...
       google.maps.event.addListener(marker, 'click', (function(marker, i) {
         return function() {
           ...
           $.post("<?php echo base_url() ?>maps/getdetailbydevid", data, function(response){
             console.log("getdetailbydevid 1");
             infowindowkedua.close();
...
function getVehicleByContractor(){
  ...
  $.post("<?php echo base_url() ?>maps/vehicleByContractor", {companyid : companyid, valuemapsoption: 9999}, function(response){
    ...
    if ( mapsOptionsValue == "showHeatmap") {
      ...
      $.post("<?php echo base_url() ?>maps/km_quickcount_newadminonly", {companyid : companyid}, function(response){
        jQuery("#loader2").hide();
            console.log("response km_quickcount_newadminonly for companyid: ", response);
            var datamuatan         = response.dataMuatan;
            var datakosongan       = response.dataKosongan;
            var datafixlimit  = response.datafixlimit;
            var arraydatamuatan    = Object.keys(datamuatan).map((key) => [String(key), datamuatan[key]]);
         ...
         for (i = 0; i < vehicle.length; i++) {
           ...
           google.maps.event.addListener(marker, 'click', (function(marker, i) {
             return function() {
               ...
               console.log("Out of hauling marker clicked ", data);
               $.post("<?php echo base_url() ?>maps/getdetailbydevid", data, function(response){
                 console.log("getdetailbydevid 5");
                 infowindowkedua.close();
        ...
        for (i = 0; i < objmapsstandard.length; i++) {
          ...
          google.maps.event.addListener(marker, 'click', (function(marker, i) {
            return function() {
              var data = {device_id : marker.id};
              $.post("<?php echo base_url() ?>maps/getdetailbydevid", data, function(response){
                console.log("getdetailbydevid 6");
                console.log("Maps standard marker onclik : ", response);
                DeleteMarkers(marker.id);
                DeleteMarkerspertama(marker.id);
