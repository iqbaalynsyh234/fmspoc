...
<script>
  $(document).ready(function() {
    ...
    function appendthecontractorlist(){
      $.post("<?php echo base_url() ?>maps/getdatacontractor_hrm", {}, function(response){
  ...
  intervalmapsstandard = setInterval(standardMapsHRM, 3000);
...
function standardMapsHRM(){
  ...
  jQuery("#loader2").show();
  $.post("<?php echo base_url() ?>maps/mapsstandard_hrm", {companyid : companyid}, function(response){
    jQuery("#loader2").hide();

    console.log("response mapsstandard_hrm : ", response);
    var vehicle             = response.data;
    var alldataforclearmaps = response.alldataforclearmaps;
    var bounds              = new google.maps.LatLngBounds();
    objmapsstandard         = vehicle;
    infowindow              = new google.maps.InfoWindow();
    console.log("objmapsstandard : ", objmapsstandard);
...
function simultangomapsstandard() {
  ...
  jQuery("#loader2").show();
  jQuery.post("<?=base_url();?>map/maps_hrm", {companyid : companyid}, function(response) {
    jQuery("#loader2").hide();

    console.log("response maps_hrm simultan : ", response);
    var vehicle             = response.data;
    var bounds              = new google.maps.LatLngBounds();
    ...
    for (i = 0; i < objmapsstandardsimultan.length; i++) {
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          var data = {device_id : marker.id};
          console.log("getdetailbydevid 4 data : ", data);
          $.post("<?php echo base_url() ?>maps/getdetailbydevid", data, function(response){
