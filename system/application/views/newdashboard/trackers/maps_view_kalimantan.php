            ...
            <div class="row">
              <div class="col-md-12">
                <div class="input-group">
                    ...
                    <button type="button" name="button" class="btn btn-primary btn-sm" style="margin-left:2%;" onclick="showTableTracking();">Table Tracking</button>
                </div>
              </div>
            </div> <br>
            ...
            <div class="row">
              <div class="col-md-9">
                <input type="hidden" id="valueMode" value="0">
                  ...
                  <div id="tableShow" style="width: 100%; max-height: 400px; overflow-x: auto; display:none;">
                    <table class="table table-striped" class="display" class="full-width">
                        <thead>
                            <tr>
                                <th></th>
                                <th>No</th>
              ...
              <div class="col-md-3">
                <div class="panel" id="panel_form">
                  ...
                  <div class="panel-body" id="bar-parent10">
                    ...
                    <div id="summaryalertcontent" style="margin-top: 1%; overflow-x: auto; max-height: 250px;"></div>
                  </div>
              </div>
            </div>
  ...
  function initialize(){
    ...
    for (i = 0; i < obj.length; i++) {
      ...
      google.maps.event.addListener(marker, 'click', (function(marker, i) {
        return function() {
          ...
          $.post("<?php echo base_url() ?>maps/getdetailbydevid", data, function(response){
            ...
            var voltasefull    = obj[i].vehicle_fuel_volt;
            var tankfullcap    = obj[i].vehicle_fuel_capacity;
            var percentagefull = 100;
            var currentvolt, percenvoltase, percenvoltase1, sisaliterbensin, numvolt, roundvolt;
            if (obj[i].vehicle_mv03 != 0000) {
              var numvolt     = Number(obj[i].auto_last_fuel);
              ...
              // console.log("Vehicle MV03 Not 0000 : ", obj[i].vehicle_no+" - "+obj[i].vehicle_name);
      ...
      htmlautoupdaterow += '<tr id="rowid_'+obj[i].vehicle_device+'">'+
                              ...
                              '<td style="font-size:12px; vertical-align:top" id="position_'+obj[i].vehicle_device+'"><span style="color:blue;">Position : '+obj[i].auto_last_position + "</span> <br> GPS Time : " + obj[i].auto_last_update + "<br>" + "Coord : " + "<a href='http://maps.google.com/maps?z=12&t=m&q=loc:"+obj[i].auto_last_lat+','+obj[i].auto_last_long+"' target='_blank'>" + obj[i].auto_last_lat + ", " + obj[i].auto_last_long + "</a>"+ "<br>" + "Engine : " + obj[i].auto_last_engine + "<br>" + "Fuel : " + sisaliterbensin + "<br>" + "Speed : " + rounded + ' Kph </br> Jalur : '+obj[i].auto_last_road+' </br> Ritase : '+obj[i].auto_last_ritase+'</br>Direction : '+courserounded+'</td>'+
...
// KLIK DARI TABLE
  function forgetcenter(deviceid){
    ...
    $.post("<?php echo base_url() ?>maps/getdetailbydevid_0", data, function(response){
      ...
      var voltasefull    = response[0].vehicle_fuel_volt;
      var tankfullcap    = response[0].vehicle_fuel_capacity;
      var percentagefull = 100;
      var currentvolt, percenvoltase, percenvoltase1, sisaliterbensin, numvolt, roundvolt;
      if (response[0].vehicle_mv03 != 0000) {
        var numvolt     = Number(response[0].auto_last_fuel);
        var roundvolt   = numvolt.toFixed(2);
        currentvolt     = Number(roundvolt);
        ...
        // console.log("Vehicle MV03 Not 0000 : ", response[0].vehicle_no+" - "+response[0].vehicle_name);
        percenvoltase1     = currentvolt * (percentagefull / voltasefull); // persentase yg didapat dari perubahan voltase;
        var percenvoltase2 = Number(percenvoltase1);
        var percenvoltase3 = percenvoltase2.toFixed(0);
        percenvoltase      = Number(percenvoltase3);
...
function getalertnow(devid){
  ...
  $.post("<?php echo base_url() ?>securityevidence/realtimealert", {device : devid}, function(response){
    if (response.sizedata != 0) {
      ...
      // console.log("else reversearray : ", reversearray);
      // console.log("else reversearraysummary : ", reversearraysummary);
      // console.log("else reversearraysummary length : ", reversearraysummary.length);

      // var alertname = '<span style="font-size: 12px; color:green; margin-left:1%;">' + "BKA-105" +" : </span>"+
      //                             '<span style="font-size: 12px; color:red;">'+ "Overspeed" +'</span> :  <span style="font-size:12px;">'+ "10:57" +' </span> <span style="font-size:12px;"><a style="font-size: 12px;" href="http://maps.google.com/maps?z=12&t=m&q=loc'+"-324413"+','+"1000023123"+'" target="_blank">'+"KM 10"+'</span></a> </br>';

        for (var i = 0; i < reversearraysummary.length; i++) {
          htmlsummary += '<tr>';
            // htmlsummary += '<td style="font-size: 12px;">'+ (j+1) +'. </td>';
            htmlsummary += '<td style="font-size: 12px; color:green;">'+ "BKA-10"+(i+1) +' </td>';
            htmlsummary += '<td><span style="font-size:12px; color: red;">'+ " Overspeed"   + '</span>' + ' ' + ' <span style="font-size: 12px;"> ' + " 10:5"+(i+2) +'</span></td>';
            htmlsummary += '<td style="font-size:12px;"><a style="font-size:12px;" href="http://maps.google.com/maps?z=12&t=m&q=loc'+"-302321"+','+"1000023123"+'" target="_blank">'+ " KM 1"+(i+1) +' </a></td> </br>';
          htmlsummary += '</tr>';
        }
        $("#summaryalertcontent").html(htmlsummary);

        if (realtimealertarraysummary.length > 10) {
          console.log("masuk");
