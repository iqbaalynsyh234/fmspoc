...
function page(p)
	{
		...
		jQuery.post("<?=base_url();?>production/search", jQuery("#frmsearch").serialize(),
			function(r)
			{
				if (r.error) {
					alert(r.message);
					...
					<div class="col-md-12 col-sm-12">
                            <div class="panel" id="panel_form">
                              <div class="card-header">
								<center><font size="5px"><b>HAULING PRODUCTION DASHBOARD (DRAFT)</font> </b><br /><small><span id="periode_show"></span></small></center>
							  </div>
                                <div class="panel-body" id="bar-parent10">
                                    	<div class="form-group row">
											...
											<div class="col-lg-3 col-md-3">
												<select id="port" name="port" class="form-control select2" >
                                                    <option value="all">All Port</option>
													<?php
														$cport = count($rport);
															for($i=0;$i<$cport;$i++){
																echo "<option value='" . $rport[$i]->port_name ."' " . $selected . ">" . $rport[$i]->port_name."</option>";
																}
													?>
												</select>
											</div>
