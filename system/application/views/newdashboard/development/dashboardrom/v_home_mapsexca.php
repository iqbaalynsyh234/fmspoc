...
<script type="text/javascript">
  function searchradiusnow(){
    var forshowhideradius = $("#forshowhideradius").val();
    var radius_for_exca1 = $("#radius_for_exca1").val();
    var radius_for_exca2 = $("#radius_for_exca2").val();
    var radius_for_exca3 = $("#radius_for_exca3").val();
    var radius_for_dt = $("#radius_for_dt").val();
    var search_1, search_2;

      if (forshowhideradius == 0 || forshowhideradius == 1) {
        search_1 = radius_for_exca1;
        search_2 = radius_for_exca2;
      }else {
        search_1 = radius_for_exca3;
        search_2 = radius_for_dt;
      }
      ...
      jQuery("#loader3").show();
      $("#resultradius").html("");
      jQuery.post("<?php echo base_url() ?>development/getradius", data, function(r){
        jQuery("#loader3").hide();
        console.log("response : ", r);
        $("#resultradius").html("Jarak antara <b>" + r.unit_1 + "</b> ("+r.gps_time_1+" : "+r.coord_1+") dengan <b>" + r.unit_2 + "</b> ("+r.gps_time_2+" : "+r.coord_2+") = <b>" + r.meter + " Mtr</b>");
      }, "json");
  }
</script>
      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body">
            ...
            <div class="row">
              <div class="col-md-3">
                <select class="form-control select2" name="forshowhideradius" id="forshowhideradius" onchange="onchangeradius()">
                  <option value="0">-- Select Radius For</option>
                  <option value="1">Exca to Exca</option>
                  <option value="2">Exca to DT</option>
                </select>
              </div>

              <div class="col-md-3" id="radius1" style="display:none;">
                <select class="form-control select2" name="radius_for_exca1" id="radius_for_exca1"></select>
              </div>

              <div class="col-md-3" id="radius2" style="display:none;">
                <select class="form-control select2" name="radius_for_exca2" id="radius_for_exca2"></select>
              </div>

              <div class="col-md-3" id="radius3" style="display:none;">
                <select class="form-control select2" name="radius_for_exca3" id="radius_for_exca3"></select>
              </div>

              <div class="col-md-3" id="radius4" style="display:none;">
                <select class="form-control select2" name="radius_for_dt" id="radius_for_dt"></select>
              </div>

              <div class="col-md-3" id="btnsearchradiusshowhide" style="display:none;">
                <div class="btn btn-md btn-primary" name="btnsearchradius" id="btnsearchradius" onclick="searchradiusnow()">Process</div>
            ...
            <div class="row">
              <div class="col-md-12">
                <div id="resultradius"></div>
              </div>
            </div>

            <!-- <div class="row" id="showhideexcatoexca" style="display:none;">
              <div class="col-md-3">
                <select class="form-control select2" name="radius_for_exca1" id="radius_for_exca1"></select>
              </div>
            ...
            <div class="row" id="showhideexcatodt" style="display:none;">
              <div class="col-md-3">
                <select class="form-control select2" name="radius_for_exca3" id="radius_for_exca3"></select>
              </div>
  ...
  $(document).ready(function() {
    ...
    function appendthevehiclelist(){
      ...
      var html = "";
        ...
        html += '<option value="0">--Exca List</option>';
        html += '<?php for ($i=0; $i < sizeof($vehicledata); $i++) {?>';
          html += '<option value="<?php echo $vehicledata[$i]['vehicle_device'] ?>"><?php echo $vehicledata[$i]['vehicle_no'] ?></option>';
        html += '<?php } ?>';

        $("#searchnopol").html(html);
        $("#radius_for_exca1").html(html);
        $("#radius_for_exca2").html(html);
        $("#radius_for_exca3").html(html);
    }
  });
...
function standardMaps(){
  ...
  jQuery("#loader2").show();
  $.post("<?php echo base_url() ?>development/mapsstandard", {companyid : companyid}, function(response){
    jQuery("#loader2").hide();
    ...
    htmlchangeVList.options[htmlchangeVList.options.length] = new Option('Exca List', "0");
    for (var i = 0; i < vehicle.length; i++) {
      if (vehicle[i].vehicle_typeunit == 1) {
        htmlchangeVList.options[htmlchangeVList.options.length] = new Option(vehicle[i].vehicle_no, vehicle[i].vehicle_device);
      }
    }

    var htmldt = "";
    $("#radius_for_dt").html("");
    htmldt += '<option value="0">--Vehicle List</option>';

    for (i = 0; i < objmapsstandard.length; i++) {

          if (objmapsstandard[i].vehicle_typeunitname == "DT") {
              htmldt += '<option value="'+objmapsstandard[i].vehicle_device+'">'+ objmapsstandard[i].vehicle_no+'</option>';
          }
        $("#radius_for_dt").html(htmldt);
...
function simultangomapsstandard() {
  ...
  // jQuery.post("<?=base_url();?>map/mapsstandardexcalastinfoall", {companyid : companyid}, function(response) {
    jQuery.post("<?=base_url();?>development/mapsexcasimultan", {companyid : companyid}, function(response) {
    jQuery("#loader2").hide();
    ...
    for (var j = 0; j < response.alldataforclearmaps.length; j++) {
      DeleteMarkerspertama(response.alldataforclearmaps[j].vehicle_device);
      DeleteMarkers(response.alldataforclearmaps[j].vehicle_device);
      DeleteMarkerspertama(response.alldataforclearmaps[j].vehicle_id);
      DeleteMarkers(response.alldataforclearmaps[j].vehicle_id);
    }
    ...
    for (i = 0; i < objmapsstandardsimultan.length; i++) {

        if (objmapsstandardsimultan[i].vehicle_typeunitname == "DT") {
            htmldt += '<option value="'+objmapsstandardsimultan[i].vehicle_device+'">'+ objmapsstandardsimultan[i].vehicle_no+'</option>';
        }
        $("#radius_for_dt").html(htmldt);
...
function onchangeradius(){
  var onchangeradius = $("#forshowhideradius").val();
    if (onchangeradius == 0 || onchangeradius == 1) {
      $("#btnsearchradiusshowhide").show();
      $("#radius1").show();
      $("#radius2").show();
      $("#radius3").hide();
      $("#radius4").hide();

      // $("#showhideexcatoexca").show();
      // $("#showhideexcatodt").hide();
    }else {
      $("#btnsearchradiusshowhide").show();
      $("#radius1").hide();
      $("#radius2").hide();
      $("#radius3").show();
      $("#radius4").show();
      // $("#showhideexcatoexca").hide();
      // $("#showhideexcatodt").show();
    }
