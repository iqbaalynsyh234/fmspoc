...
jQuery(document).ready(
		function()
		{
			jQuery("#export_xcel").click(function()
			{
				...
				var violationmasterselect = jQuery("#violationmasterselect").val();
				...
				var data = {
					company : company,
					vehicle : vehicle,
					violationmasterselect : violationmasterselect,
					periode : periode,
					startdate : startdate,
					enddate : enddate,
				};
				...
				jQuery("#loader2").show();
				jQuery.post("<?php echo base_url() ?>development/search_violationtablehistorikalphpexcel", data, function(response){
					console.log("response result : ", response);
					jQuery("#loader2").hide();
					var $a = $("<a>");
					$a.attr("href",response.file);
					$("body").append($a);
					$a.attr("download","Violation_historikal.xls");
					$a[0].click();
					$a.remove();
      ...
      <div class="row">
        <div class="col-md-12" id="tablevehicles">
          <div class="panel" id="panel_form">
            <header class="panel-heading" id="result-violation">RESULT VIOLATION TABLE -  DEVELOPMENT</header>
                        ...
                        <?php for ($i=0; $i < sizeof($data); $i++) {?>
                          <tr>
                           ...
                           <td>
                            ...
                            <td>
                              <?php
                                $statussentetele = $data[$i]['status_sent_tele'] ;
                                  if ($statussentetele == 0) {
                                    echo "Not Sent";
                                  }else {
                                    echo "Sent";
                                  }
