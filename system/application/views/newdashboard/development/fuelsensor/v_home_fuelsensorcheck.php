    ...
    <div class="row">
      <div class="col-md-12 col-sm-12">
        <div class="panel" id="panel_form">
          <header class="panel-heading" id="fuel-sensor">FUEL SENSOR HISTORY - DEVELOPMENT</header>
    ...
    if (privilegecode == 5 || privilegecode == 6) {
      ...
      html += '<?php for ($i=0; $i < sizeof($vehicledata); $i++) {?>';
        ...
        if (vCompany == user_company) {
          html += '<option value="<?php echo $vehicledata[$i]['vehicle_device'] ?>"><?php echo ($i+1).'. '. $vehicledata[$i]['vehicle_no'] ?></option>';
        }
      html += '<?php } ?>';
    }else {
      html += '<option value="all">--Vehicle List</option>';
	 ...
	 function page(p) {
		...
		jQuery("#loader2").show();
    jQuery("#result").hide();
		jQuery.post("<?= base_url(); ?>development/search_fuelsensor_history", jQuery("#frmsearch").serialize(),
			function(r) {
        // console.log("response search_fuelsensor_history : ", r);
				if (r.error) {
					alert(r.message);
					jQuery("#loader2").hide();
					jQuery("#result").hide();
