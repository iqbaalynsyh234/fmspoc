...
<script>
  ...
  function simultango() {
    ...
    // console.log("timer_list di simultango : ", '<?php echo $this->config->item('timer_list ');?>');
      jQuery.post("<?=base_url();?>map/lastinfonew", {
          ...
          // lasttime: 100,
          lasttime: obj[objectnumberfix].auto_last_update,
  ...
  function add_new_markers1(value) {
    ...
    console.log("value.driver : ", value.driver);

      if (value.vehicle_device == "69969039633231@TK510") {
        if (value.driverimage != 0) {
          console.log("sikon 2 in if driverimage");
          // var showdriver   = "<a href='#' onclick='getmodaldriver("+datadriver[0]+");'>"+ datadriver[1] +"</a>";
          var detaildriver = '<img src="<?php echo base_url().$this->config->item("dir_photo");?>'+value.driverimage+'" width="100px;" height="100px;"> </br>' + value.driver ;
          $("[id='driver_"+value.vehicle_device+"']").html(detaildriver);
        }else {
          var detaildriver = value.driver + ' </br> No Driver Image';
          $("[id='driver_"+value.vehicle_device+"']").html(detaildriver);
        }
      }else {
        if (value.driver){
          console.log("sikon 0 in else");
          datadriver   = value.driver;
          if (datadriver.includes("-")){
            datadriver   = value.driver.split('-');
            var namadriverfix = datadriver[1];
          }else {
            var namadriverfix = value.driver;
          }
           if (value.driverimage) {
             console.log("sikon 1 in else");
             if (value.driverimage != 0) {
               console.log("sikon 2 in else");
               // var showdriver   = "<a href='#' onclick='getmodaldriver("+datadriver[0]+");'>"+ datadriver[1] +"</a>";
               var detaildriver = '<img src="<?php echo base_url().$this->config->item("dir_photo");?>'+value.driverimage+'" width="100px;" height="100px;"> </br>' + namadriverfix ;
               $("[id='driver_"+value.vehicle_device+"']").html(detaildriver);
             }else {
               var detaildriver = namadriverfix + ' </br> No Driver Image';
               $("[id='driver_"+value.vehicle_device+"']").html(detaildriver);
             }
           }else {
             var detaildriver = namadriverfix + ' </br> No Driver Image';
          ...
          var detaildriver = value.driver + ' </br> Unknown';
          $("[id='driver_"+value.vehicle_device+"']").html(detaildriver);
        }
      }
