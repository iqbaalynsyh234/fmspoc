...
<body class="page-header-fixed sidemenu-closed-hidelogo page-content-white page-md header-red white-sidebar-color logo-white page-footer-fixed">
  <div class="page-wrapper">
    ...
    <div class="page-footer">
      <div class="page-footer-inner"> 2022 &copy;
        <a href="#" target="_blank" class="makerCss">
          <?php
            if (isset($this->sess->user_company_alias)) {
              $pjo = $this->sess->user_company_alias;
              echo $pjo;
            }else {
              echo "";
            }
  ...
  window.onload = function() {
    ...
    var url = '<?php echo base_url() ?>maintenance/getforalert';
      $.post(url, {}, function(response){
        // console.log("response getforalert : ", response);

        var totaldata     = response.totaldata;
        var databreakdown = response.data;

          if (totaldata > 0) {
            $("#tablebreakdownreport").show();
            for (var i = 0; i < totaldata; i++) {
              var breakdownrow = '<tr>';
                breakdownrow += '<td>' + (i+1) + '. </td>';
                breakdownrow += '<td> ' +databreakdown[i].breakdown_vehicle_no +'</td>';
                breakdownrow += '<td> ' +databreakdown[i].breakdown_kat_name + '</td>';
                breakdownrow += '<td> ' +databreakdown[i].breakdown_start_time + '</td>';
              breakdownrow += '</tr>';
              $("#rowbreakdownreport").before(breakdownrow);
            }
          }else {
            $("#tablebreakdownreport").hide();
                var breakdownrow = '<tr>';
                    breakdownrow += '<td>Data Not Available </td>';
                    breakdownrow += '<td>Data Not Available </td>';
                    breakdownrow += '<td>Data Not Available </td>';
                    breakdownrow += '<td>Data Not Available </td>';
                  breakdownrow += '</tr>';
