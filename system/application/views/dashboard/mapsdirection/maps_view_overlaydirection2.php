...
<script>
    ...
    // console.log("timer_list di simultango : ", '<?php echo $this->config->item('timer_list ');?>');
      jQuery.post("<?=base_url();?>map/lastinfonew", {
        ...
        function(r) {
          ...
          move_markers(r.vehicle);
        }, "json");
  }

  function move_markers(value){
    console.log("value in move_markers : ", value);
    var totalcoord = value.gps;
    var arraycoord = [];
    for (var i = 0; i < totalcoord.length; i++) {
      var latfix = value.gps[i].gps_latitude_real_fmt;
      var lngfix = value.gps[i].gps_longitude_real_fmt;
      arraycoord.push({
        location: latfix+","+lngfix,
        stopover: false
      });
    }
    animate_marker(value, arraycoord);
    console.log("arraycoord : ", arraycoord);
  }

  function animate_marker(value, coord){
    var totalvalue         = value.gps.length;
    var coordstartfix      = coord[0].location;
    var coordstartsplitfix = coordstartfix.split(",");
    // console.log("coordstartfix : ", coordfix);
    // console.log("coordstartsplitfix : ", );
    // var map;
    //start position
     const startPosition ={lat:coordstartsplitfix[0], lng:coordstartfix[0]}
     // let map = new google.maps.Map(document.getElementById("mapsnya"), {
     //       center: new google.maps.LatLng(parseFloat(obj[0].auto_last_lat), parseFloat(obj[0].auto_last_long)),
     //       zoom: 13,
     //       mapTypeId: google.maps.MapTypeId.SATELLITE,
     //       options: {
     ...
     if (coord.length > 0) {
       for (var j = 1; j < coord.length; j++) {
           var coordnextfix      = coord[j].location;
           var coordnextsplitfix = coordnextfix.split(",");
           pathCoordinate.push({
              lat: parseFloat(coordnextsplitfix[0]),
              lng: parseFloat(coordnextsplitfix[1])
           });
       }
     }else {
    ...
    console.log("pathCoordinate : ", pathCoordinate);
    var statusengine = "";
  ...
  var num         = Number(value.gps[totalvalue-1].gps_speed_fmt);
  var roundstring = num.toFixed(0);
  var rounded     = Number(roundstring);
  ...
  var gps_status = "";
  if (value.gps[totalvalue-1].gps_status == "A" || value.gps[totalvalue-1].gps_status == "V") {
    gps_status = "Good";
  }else {
    gps_status = "Not Good";
  }

  var courseround   = Number(value.gps[totalvalue-1].gps_course);
  var coursestring  = courseround.toFixed(0);
  var courserounded = Number(coursestring);


  var titlemarker  = "";
  var addresssplit = value.gps[totalvalue-1].georeverse.display_name.split(" ");
  var inarea       = value.gps[totalvalue-1].georeverse.display_name.split(",");
  ...
  markernya = new google.maps.Marker({
    map: map,
    icon: icon,
    position: new google.maps.LatLng(parseFloat(value.gps[totalvalue-1].gps_latitude_real_fmt), parseFloat(value.gps[totalvalue-1].gps_longitude_real_fmt)),
    title: value.vehicle_no + ' - ' + value.vehicle_name,
    // + ' - ' + value.vehicle_name + value.driver + "\n" +
    //   "GPS Time : " + value.gps.gps_date_fmt + " " + value.gps.gps_time_fmt + "\n" + value.gps.georeverse.display_name + "\n" + value.gps.gps_latitude_real_fmt + ", " + value.gps.gps_longitude_real_fmt + "\n" +
    //   "Speed : " + value.gps.gps_speed + " kph",
  ...
  icon.rotation = Math.ceil(value.gps[totalvalue-1].gps_course);
  markernya.setIcon(icon);
  markerss.push(markernya);

  // CALL TO MOVE MARKER
  thistransition(markernya, pathCoordinate);
      ...
      titlemarker += '<table class="table" style="font-size:12px;">';
        titlemarker += '<tr>';
          ...
          titlemarker += '<td>';
            ...
            titlemarker += 'Gps Time : ' + value.gps[totalvalue-1].gps_date_fmt + " " + value.gps[totalvalue-1].gps_time_fmt + '</br>';
            titlemarker += 'Position : ' + geofencestatus + addressfix + '</br>';
            titlemarker += 'Coord : ' + value.gps[totalvalue-1].gps_latitude_real_fmt + ", " + value.gps[totalvalue-1].gps_longitude_real_fmt + '</br>';
  ...
  google.maps.event.addListener(markernya, 'click', function(evt){
    ...
    // }
      ...
      var center = {lat : parseFloat(value.gps[totalvalue-1].gps_latitude_real_fmt), lng: parseFloat(value.gps[totalvalue-1].gps_longitude_real_fmt)};
  ...
  var deltaLng;
  var iloopnya = 0;
  function thistransition(markernya, coordfix){
    console.log("coordfix in thistransition : ", coordfix);
    // var totalcoord = coordfix.length-1;
    //
    for (var i = 0; i < coordfix.length; i++) {
      for (var j = 1; j < i; j++) {
        iloopnya = 0;
        deltaLat = (coordfix[j][1] - coordfix[i][0])/100;
        deltaLng = (coordfix[j][1] - coordfix[i][1])/100;
        moveMarker(markernya);
      }
    }
  ...
  function moveMarker(){
    position[0] += deltaLat;
    position[1] += deltaLng;
    console.log("position in movemarker : ", position[0]+" - "+position[1]);
