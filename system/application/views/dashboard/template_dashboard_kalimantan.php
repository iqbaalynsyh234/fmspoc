  ...
  window.onload = function() {
    ...
    function getalertalways() {
        $.post("<?php echo base_url()?>devicealert/getallalert", {}, function(response){
          ...
          // console.log("data alert terbaru : ", data);
          if (data.length > 1) {
            // console.log("alert lebih dari 1 : ", data);
          }
          ...
          var htmlnotif = "";
            if (code == 200) {
              ...
              htmlnotif += '<ul class="dropdown-menu-list small-slimscroll-style" data-handle-color="#637283">';
                ...
                // UNTUK REALTIME ALERT DI MAPS ALL KALIMANTAN START
                if (summaryarray.length == 0) {
                  // console.log("summaryarray = 0");
                  summaryarray.push(data);
                  // UNTUK GABUNG KE REALTIME ALERT DI MAPS
                  var alertfromdevicealertforsummary = "";
                    alertfromdevicealertforsummary += '<table class="table">';
                    for (var i = 0; i < summaryarray.length; i++) {
                      var alert_name    = summaryarray[i][0].vehicle_alert;
                      var devicealertfix = alert_name in devalertarray ? devalertarray[alert_name] : alert_name;
                      alertfromdevicealertforsummary += '<tr>';
                        alertfromdevicealertforsummary += '<td style="font-size: 12px; color:green; text-align:center">'+ summaryarray[i][0].vehicle_no +'</td>';
                        alertfromdevicealertforsummary += '<td><span style="font-size:12px; color:red; text-align:center">'+ devicealertfix +'<td>';
                        alertfromdevicealertforsummary += '<td><span style="font-size:12px; color:red; text-align:center">'+ summaryarray[i][0].gps_speed +' Kph<td>';
                        alertfromdevicealertforsummary += '<td><span style="font-size:12px; color:red; text-align:center">Limit :'+ summaryarray[i][0].gps_speed_limit +' Kph<td>';
                        alertfromdevicealertforsummary += '<td><span style="font-size:12px; color:red; text-align:center">'+ summaryarray[i][0].vehicle_alert_datetime +'<td>';
                        alertfromdevicealertforsummary += '<td style="font-size:12px; text-align:center"><a href="http://maps.google.com/maps?z=12&t=m&q=loc:'+summaryarray[i][0].vehicle_lat+','+summaryarray[i][0].vehicle_lng+'" target="_blank">'+summaryarray[i][0].position+'</a></td>';
                      alertfromdevicealertforsummary += '</tr>';
                    }
                  // UNTUK GABUNGAN DEVICE ALERT KE REALTIME ALERT
                  alertfromdevicealertforsummary += '</table>';

                  var alertfromdevicealert = "";
                    alertfromdevicealert += '<table class="table">';
                    for (var i = 0; i < data.length; i++) {
                      var alert_name    = data[i].vehicle_alert;
                      var devicealertfix = alert_name in devalertarray ? devalertarray[alert_name] : alert_name;
                      alertfromdevicealert += '<tr>';
                        alertfromdevicealert += '<td style="font-size: 12px; color:green; text-align:center">'+ data[i].vehicle_no +'</td>';
                        alertfromdevicealert += '<td><span style="font-size:12px; color:red; text-align:center">'+ devicealertfix +'<td>';
                        alertfromdevicealert += '<td><span style="font-size:12px; color:red; text-align:center">'+ data[i].gps_speed +' Kph<td>';
                        alertfromdevicealert += '<td><span style="font-size:12px; color:red; text-align:center">Limit :'+ data[i].gps_speed_limit +' Kph<td>';
                        alertfromdevicealert += '<td><span style="font-size:12px; color:red; text-align:center">'+ data[i].vehicle_alert_datetime +'<td>';
                        alertfromdevicealert += '<td style="font-size:12px; text-align:center"><a href="http://maps.google.com/maps?z=12&t=m&q=loc:'+data[i].vehicle_lat+','+data[i].vehicle_lng+'" target="_blank">'+data[i].position+'</a></td>';
                      alertfromdevicealert += '</tr>';
                    }
                  // UNTUK GABUNGAN DEVICE ALERT KE REALTIME ALERT
                  alertfromdevicealert += '</table>';

                  playsound();
                  $("#alertfromdevicealert").html(alertfromdevicealert);
                  $("#summaryalertfromdevicealert").html(alertfromdevicealertforsummary);
                }else {
                  // console.log("summaryarray > 0 tgl sama : ", summaryarray);
                  var lastdata         = summaryarray[summaryarray.length - 1];
                  var datefromlastdata = lastdata[0].vehicle_alert_datetime;
                  var datefromnewdata  = data[0].vehicle_alert_datetime;


                  if (datefromlastdata != datefromnewdata) { // == digunakan untuk develop, != untuk live
                    // console.log("data sama : ");
                    // console.log("datefromlastdata : ", datefromlastdata);
                    // console.log("datefromnewdata : ", datefromnewdata);
                    // console.log("tanggal tidak sama : ");
                    summaryarray.push(data);
                    // var summaryarrayreverse = summaryarray.reverse();
                    // console.log("summaryarray jika tgl tdk sama : ", summaryarray);
                    // console.log("summaryarrayreverse : ", summaryarrayreverse);
                    // UNTUK GABUNG KE REALTIME ALERT DI MAPS
                    var alertfromdevicealertforsummary = "";
                      alertfromdevicealertforsummary += '<table class="table">';
                      for (var i = 0; i < summaryarray.length; i++) {
                ...
                // UNTUK REALTIME ALERT DI MAPS ALL KALIMANTAN END
            }else {
              htmlnotif += '<li>'+
